...
> 原文链接：<https://leetcode.cn/circle/discuss/tXLS3i/>
...
![动态规划算法题 DP题单 动态规划题单 入门动态规划题目 动态规划新手教程 力扣DP 力扣动态规划 leetcode动态规划 leetcode dp 灵茶山艾府 灵神 灵神题单](./pics/tXLS3i/dp-2.jpg)
...
- [3984. 可整除游戏](https://leetcode.cn/problems/divisible-game/) 1944
...
- [3981. 统计从两个字符串形成目标字符串的不同方案数](https://leetcode.cn/problems/count-distinct-ways-to-form-target-from-two-strings/) 2128
...
- [3952. 下标覆盖处的最大总和](https://leetcode.cn/problems/maximum-total-value-of-covered-indices/) 1763
...
- [3628. 插入一个字母的最大子序列数](https://leetcode.cn/problems/maximum-number-of-subsequences-after-one-inserting/) 1754
- [3983. 一次替换后的子序列](https://leetcode.cn/problems/subsequence-after-one-replacement/) 1755
...
- [3980. 变换二进制字符串的最少操作次数](https://leetcode.cn/problems/minimum-operations-to-transform-binary-string/) 1846 做法不止一种
- [3196. 最大化子数组的总成本](https://leetcode.cn/problems/maximize-total-cost-of-alternating-subarrays/) 1847 做法不止一种
...
- [3976. 乘以系数后最大子数组和](https://leetcode.cn/problems/maximum-subarray-sum-after-multiplier/) 1981
...
- [3989. 网格中保持一致的最大列数](https://leetcode.cn/problems/maximum-consistent-columns-in-a-grid/) 约 1800
- [368. 最大整除子集](https://leetcode.cn/problems/largest-divisible-subset/) 约 1850
- [2901. 最长相邻不相等子序列 II](https://leetcode.cn/problems/longest-unequal-adjacent-groups-subsequence-ii/) 1899
...
- [3995. 转换字符串的最小成本 III](https://leetcode.cn/problems/minimum-cost-to-convert-string-iii/) 2101
...
- [4008. 击败所有怪物的最小初始强度](https://leetcode.cn/problems/minimum-initial-strength-to-defeat-all-monsters/) 1776
...
- [4016. 两个不重叠子正方形的最大面积](https://leetcode.cn/problems/maximum-area-of-two-non-overlapping-square-submatrices/) 1959
...
- [4009. 最小化最大可能等待时间](https://leetcode.cn/problems/minimum-possible-maximum-waiting-time/) 2499
...
- [从集合论到位运算，常见位运算技巧分类总结](./06-从集合论到位运算)
...
def digitDP(low: int, high: int, target: int) -> int:
    ...
    def dfs(i: int, cnt0: int, limit_low: bool, limit_high: bool) -> int:
        ...
        # 通过 limit_low 和 i 可以判断能否不填数字，dfs 无需 is_num 参数
...
class Solution {
    ...
    private long dfs(int i, int cnt0, boolean limitLow, boolean limitHigh, char[] lowS, char[] highS, int target, long[][] memo) {
        ...
        // 通过 limitLow 和 i 可以判断能否不填数字，dfs 无需 isNum 参数
        // 如果前导零不影响答案，去掉这个 if block
        if (limitLow && i < diff) {
            // 不填数字，上界不受约束
...
long long digitDP(long long low, long long high, int target) {
    ...
    auto dfs = [&](this auto&& dfs, int i, int cnt0, bool limit_low, bool limit_high) -> long long {
        ...
        // bool is_num = !limit_low || i > diff_lh;
        for (; d <= hi; d++) {
            // 统计 0 的个数
...
- [3966. 统计范围内的好整数](https://leetcode.cn/problems/count-good-integers-in-a-range/) 2006
...
- [3954. 区间内的兼容数字之和 I](https://leetcode.cn/problems/sum-of-compatible-numbers-in-range-i/) 非暴力做法
...
前置题单：[单调栈（矩形系列/字典序最小/贡献法）](./03-单调栈)
...
- [3956. 非重叠子数组最大和 I](https://leetcode.cn/problems/maximum-sum-of-m-non-overlapping-subarrays-i/) 2061
- [1140. 石子游戏 II](https://leetcode.cn/problems/stone-game-ii/) 做到 $\mathcal{O}(n^2)$
...
1. 线性 DP（常系数齐次线性递推），转移系数写在第一行，其余行 $m_{i+1,i} = 1$，例如 [70](https://leetcode.cn/problems/climbing-stairs/)，[1137](https://leetcode.cn/problems/n-th-tribonacci-number/)。[讲解（含模板代码）](https://leetcode.cn/problems/climbing-stairs/solutions/2560716/jiao-ni-yi-bu-bu-si-kao-dong-tai-gui-hua-7zm1/)。
2. 多维 DP 或者状态机 DP，转移系数可表示为一个 $k\times k$ 大小的矩阵，例如 [935](https://leetcode.cn/problems/knight-dialer/)。[讲解（含模板代码）](https://leetcode.cn/problems/knight-dialer/solutions/3004116/jiao-ni-yi-bu-bu-si-kao-dpcong-ji-yi-hua-x06l/)。
...
[讲解](https://leetcode.cn/problems/minimum-partition-score/solutions/3893573/hua-fen-xing-dp-xie-lu-you-hua-tu-bao-yo-5cb0/)
...
[讲解](https://leetcode.cn/problems/maximum-sum-of-m-non-overlapping-subarrays-ii/solution/la-ge-lang-ri-song-chi-wqs-er-fen-python-m2iw/)
...
- [3957. 非重叠子数组最大和 II](https://leetcode.cn/problems/maximum-sum-of-m-non-overlapping-subarrays-ii/) 2611
...
- [1563. 石子游戏 V](https://leetcode.cn/problems/stone-game-v/) 单调性优化，可以做到 $\mathcal{O}(n^2)$
- [3181. 执行操作可获得的最大总奖励 II](https://leetcode.cn/problems/maximum-total-reward-using-operations-ii/) 2688 bitset 优化
...
关于**倍增算法**，见 [题单](./13-链表二叉树与回溯) 的「§3.8 最近公共祖先（LCA）、倍增算法」。
...
- [3967. 任务完成时间 II](https://leetcode.cn/problems/finish-time-of-tasks-ii/)（会员题）
...
- [3977. 有限电量到达目标节点的最少时间](https://leetcode.cn/problems/minimum-time-to-reach-target-with-limited-power/) 2103
- [3543. K 条边路径的最大边权和](https://leetcode.cn/problems/maximum-weighted-k-edge-path/) 2110 bitset
...
另见[【题单】图论算法](./07-图论算法) 中的「全源最短路：Floyd」，本质是多维 DP。
...
- [926. 将字符串翻转到单调递增](https://leetcode.cn/problems/flip-string-to-monotone-increasing/) 1602
...
- [3983. 一次替换后的子序列](https://leetcode.cn/problems/subsequence-after-one-replacement/) 1755 子序列匹配
