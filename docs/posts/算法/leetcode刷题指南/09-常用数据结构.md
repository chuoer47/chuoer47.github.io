...
> 原文链接：<https://leetcode.cn/circle/discuss/mOr1u6/>

---

![数据结构题单 数据结构入门 数据结构新手教程 数据结构题目 力扣数据结构 leetcode数据结构 灵茶山艾府 灵神 灵神题单](./pics/mOr1u6/Programming.jfif)
...
- [3979. 最大有效数对和](https://leetcode.cn/problems/maximum-valid-pair-sum/) 1328
...
- [1424. 对角线遍历 II](https://leetcode.cn/problems/diagonal-traverse-ii/) 1780
...
- [3969. 求和后首尾数字相同的有效子数组 I](https://leetcode.cn/problems/valid-subarrays-with-matching-sum-digits-i/) 非暴力做法
...
推荐先阅读：[从集合论到位运算，常见位运算技巧分类总结！](./06-从集合论到位运算)
...
队列常用在 BFS 中，见 [网格图题单](./04-网格图) 和 [图论题单](./07-图论算法)。与此相比，栈常用在 DFS 中，但无需我们手动维护。
...
关于单调队列优化 DP，见 [动态规划题单](./08-动态规划) 的「**§11.3 单调队列优化 DP**」。
...
- [3962. 至多 K 次交换后最大子数组和](https://leetcode.cn/problems/maximum-subarray-sum-after-at-most-k-swaps/) 2672
...
另见 [图论题单](./07-图论算法) 中的 Dijkstra 算法。
...
更多基础题，见 [网格图题单](./04-网格图) 中的 DFS 和 [图论题单](./07-图论算法) 中的 DFS，其中大部分题目也可以用并查集实现。
...
部分题目需要用到**离散化**，推荐先完成 [1331. 数组序号转换](https://leetcode.cn/problems/rank-transform-of-an-array/)。
...
```py [sol-Python3]
# 应用：https://leetcode.cn/problems/maximum-subarray-sum-after-at-most-k-swaps/
class FenwickTree:
...
- [3962. 至多 K 次交换后最大子数组和](https://leetcode.cn/problems/maximum-subarray-sum-after-at-most-k-swaps/) 2672 前 $k$ 小之和
- [3382. 用点构造面积最大的矩形 II](https://leetcode.cn/problems/maximum-area-rectangle-with-point-constraints-ii/) 2723 静态二维数点
- [3590. 第 K 小的路径异或和](https://leetcode.cn/problems/kth-smallest-path-xor-sum/) 第 $k$ 小
...
- [3994. 划分数组的最少相邻交换次数](https://leetcode.cn/problems/minimum-adjacent-swaps-to-partition-array/) 1705 入门题（不需要树状数组）
...
- [2846. 边权重均等查询](https://leetcode.cn/problems/minimum-edge-weight-equilibrium-queries-in-a-tree/) 树上莫队 和边权范围无关的做法
...
- 关于**链表**和**二叉树**的题目，见 [链表、树与回溯](./13-链表二叉树与回溯)。
- 关于**字符串**算法，见 [字符串题单](./14-字符串)。
