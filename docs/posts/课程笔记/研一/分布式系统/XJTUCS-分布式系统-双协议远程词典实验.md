...
项目提供了一键脚本 `scripts/run_local_experiment.sh`，会自动完成以下工作：
...
::: details 方式 A：使用本地 Consul dev agent（点击展开）
...
::: details 方式 B：使用 Docker Compose（点击展开）

如果实验环境具备 Docker，可直接使用 `deploy/docker-compose.yml`：
...
- `benchmark/results/report/raw_results.json` / `.csv` / `.md`
- `benchmark/results/report/aggregate_results.json` / `.csv` / `.md`
...
![系统架构](./XJTUCS-分布式系统-双协议远程词典实验.assets/architecture.svg)
...
::: details RegistryLifecycle：注册 / 心跳 / 注销核心代码（点击展开）
...
::: details DiscoveryCache.resolve：短 TTL 缓存 + 单飞请求（点击展开）
...
::: details FrameBuffer 解帧核心代码（点击展开）
...
原始逐轮结果保存在 `benchmark/results/report/raw_results.md` 与 `benchmark/results/report/aggregate_results.md`。

### 4.3 图表

#### QPS 对比

![QPS](./XJTUCS-分布式系统-双协议远程词典实验.assets/qps.svg)

#### P95 延迟对比

![P95 Latency](./XJTUCS-分布式系统-双协议远程词典实验.assets/p95_latency.svg)

#### P99 延迟对比

![P99 Latency](./XJTUCS-分布式系统-双协议远程词典实验.assets/p99_latency.svg)

#### CPU 峰值对比

![CPU Peak](./XJTUCS-分布式系统-双协议远程词典实验.assets/cpu_peak.svg)

#### RSS 峰值对比

![RSS Peak](./XJTUCS-分布式系统-双协议远程词典实验.assets/rss_peak.svg)

#### 有效载荷大小对比

![Payload Size](./XJTUCS-分布式系统-双协议远程词典实验.assets/payload_size.svg)
...
项目共编写 `16` 个自动化测试用例，覆盖协议解析、注册中心、服务发现、容错机制、benchmark 汇总和双协议端到端调用。最终执行 `ruff check .`、`mypy .`、`pytest -q`，实际结果：
...
在 benchmark 完成后，向两个服务发送中断信号，再次查询健康实例列表，实际返回 `[]`。
