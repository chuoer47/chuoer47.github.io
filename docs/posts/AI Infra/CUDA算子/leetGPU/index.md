...
- [Medium Kernel](./Medium-kernel.md) — 金字塔归约 Reduction、online softmax 全局版（含 per-block 翻车实录）、一行一 warp 的 Softmax Attention（FA 教学版）、2D Convolution（kernel 常驻 smem 与 halo 分块两版对照）、Prefix Sum（Hillis-Steele 块内扫描与 warp shuffle 两级扫描，含函数详解与通用坑位）、Dot Product（Reduction 的乘加融合复用，两级收尾组合拳）
