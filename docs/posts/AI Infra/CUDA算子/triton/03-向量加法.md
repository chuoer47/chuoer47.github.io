...
### 2.5 这套写法不支持反向传播
wrapper 没接入 PyTorch 计算图，所以 `.backward()` 走不通。前向够用；反向是[第 07 章](07-LayerNorm.md)的事。
...
- **1D grid `(N,)`**：单维并行。向量加法这种一维数据用它：`grid = (cdiv(向量长度, BLOCK_SIZE),)`。
  ...
  ```
- **2D grid `(M, N)`**：两维并行。矩阵运算（matmul、softmax）用它：比如 `M` 行 `N` 列的输出，每个 program 算一个 `(行块, 列块)`。[第 05/06 章](04-融合Softmax.md)会见到。
  ```
  PID 排成二维：
  ...
  ```
- **3D grid `(B, H, N)`**：三维并行。更高维数据（如 batch×头×序列 的注意力，[第 08 章](08-FlashAttention.md)）用它。不过实际上常常**把几个维度合并**（如 batch×head 合成一维）退回成 2D grid，更紧凑——09 章就是这么干的。
...
> （第 04 章会出现 `grid = (num_programs, 1, 1)` 这种"看起来是 3D、实际只用一维"的写法——那是 `.warmup()` 的版本限制逼出来的，不是真三维修补，那里会再讲。）
...
pid = tl.program_id(axis=0)
```
这个 op 是 1D 的，只有 `axis=0`。`pid` 是这个 program 的唯一编号。后面靠它算"我该处理哪块数据"。（多维 op 会有 `program_id(0)`、`program_id(1)`，[第 06/09 章](08-FlashAttention.md)会见到。）
...
- `other=None`：被 mask 挡住的位置填什么值。这里 `None` 表示该位置不加载、其值未定义；后续运算照常执行（SIMT 决定了同一 warp 的核心必须做相同运算，被 mask 的 lane 也会参与），但结果会在 `tl.store` 时被 mask 丢弃，不影响最终输出。某些场景填 `0.0` 或 `-inf` 更合适（[第 04 章](04-融合Softmax.md)会看到 `-inf`）。
...
**记住那个三步循环图**。下一章我们用它解决一个真问题：为什么 softmax 要"融合"，以及怎么读 GPU 规格、算 occupancy。→ [第 04 章：融合 Softmax](04-融合Softmax.md)
