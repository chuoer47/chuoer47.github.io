...
- **warp**：最小的核心编组。Nvidia 上 **32 核为一个 warp**，AMD 上 64。**同一 warp 内所有核心必须做完全相同的运算**（这叫 SIMT，Single Instruction Multiple Threads——单指令多线程，即同一 warp 的线程在同一时刻执行同一条指令）。一个 program 里有多个 warp，数量通常由编译器或 autotune 决定（第 04 章会看到如何手动设、第 05 章会看到如何自动调）。
...
- **每个 SM 上至少跑一个 program**，能跑多少个取决于 SRAM 够不够分：
  ...
  ```
  （这里先只看 SRAM 这一限制；实际还受寄存器限制——第 04 章 occupancy 会把两者都纳入，取较小的那个。）
- 如果**单个 program 需要的数据超过 SM 的 SRAM**，会直接报错（这就是为什么后面有的内核要分块循环，而不是一口气加载整行/整列）。
- **如果同一 SM 上不同 PID 加载相同数据，它们会共享而不是各加载一份**——这是 matmul 教程里"PID 重排"能提速的根本原因（[第 05 章](05-矩阵乘法.md)）。
...
有了这些，我们就可以写第一个内核了。→ [第 03 章：向量加法——第一个 Triton 内核](03-向量加法.md)
