<template><div><h1 id="netty基础" tabindex="-1"><a class="header-anchor" href="#netty基础"><span>Netty基础</span></a></h1>
...
<h2 id="一、netty概述" tabindex="-1"><a class="header-anchor" href="#一、netty概述"><span>一、Netty概述</span></a></h2>
<h3 id="_1、原生nio存在的问题" tabindex="-1"><a class="header-anchor" href="#_1、原生nio存在的问题"><span>1、原生NIO存在的问题</span></a></h3>
<ul>
<li>
...
<h3 id="_2、netty的优点" tabindex="-1"><a class="header-anchor" href="#_2、netty的优点"><span>2、Netty的优点</span></a></h3>
<p>Netty 对 JDK 自带的 NIO 的 API 进行了封装，解决了上述问题。</p>
<ul>
<li>
<p>1、设计优雅：适用于各种传输类型的统一 API 阻塞和非阻塞 Socket；基于灵活且可扩展的事件模型，可以清晰
地分离关注点；高度可定制的线程模型 - 单线程，一个或多个线程池.</p>
</li>
<li>
<p>2、使用方便：详细记录的 Javadoc，用户指南和示例；没有其他依赖项，JDK 5（Netty 3.x）或 6（Netty 4.x）就
足够了。</p>
</li>
<li>
...
<li>
<p>5、社区活跃、不断更新：社区活跃，版本迭代周期短，发现的 Bug 可以被及时修复，同时，更多的新功能会被
加入</p>
</li>
</ul>
<h3 id="_3、netty版本说明" tabindex="-1"><a class="header-anchor" href="#_3、netty版本说明"><span>3、Netty版本说明</span></a></h3>
<ul>
<li>
<p>1、netty 版本分为 netty3.x 和 netty4.x、netty5.x</p>
</li>
<li>
<p>2、因为 Netty5 出现重大 bug，已经被官网废弃了，目前推荐使用的是 Netty4.x 的稳定版本</p>
</li>
<li>
<p>3、目前在官网可下载的版本 netty3.x netty4.0.x 和 netty4.1.x</p>
</li>
<li>
<p>4、netty <a href="https://bintray.com/netty/downloads/netty/" target="_blank" rel="noopener noreferrer">下载地址</a></p>
</li>
</ul>
<h2 id="二、netty高性能架构设计" tabindex="-1"><a class="header-anchor" href="#二、netty高性能架构设计"><span>二、Netty高性能架构设计</span></a></h2>
<h3 id="_1、线程模型介绍" tabindex="-1"><a class="header-anchor" href="#_1、线程模型介绍"><span>1、线程模型介绍</span></a></h3>
<p>不同的线程模式，对程序的性能有很大影响，为了搞清 Netty 线程模式，我们来系统的讲解下 各个线程模式，最后看看 Netty 线程模型有什么优越性。</p>
<ul>
<li>
<p>目前存在的线程模型有：传统阻塞 I/O 服务模型、Reactor 模式</p>
</li>
<li>
<p>根据 Reactor 的数量和处理资源池线程的数量不同，有 3 种典型的实现：单 Reactor 单线程、单 Reactor 多线程、主从 Reactor 多线程</p>
</li>
<li>
<p>Netty 主要基于主从 Reactor 多线程模型做了一定的改进，其中主从 Reactor 多线程模型有多个 Reactor</p>
</li>
</ul>
<h3 id="_2、传统阻塞-i-o-服务模型" tabindex="-1"><a class="header-anchor" href="#_2、传统阻塞-i-o-服务模型"><span>2、传统阻塞 I/O 服务模型</span></a></h3>
<figure><img src="/image/jvm/jvm105.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
<ul>
<li>
<p>1、当并发数很大，就会创建大量的线程，占用很大系统资源</p>
</li>
<li>
<p>2、连接创建后，如果当前线程暂时没有数据可读，该线程会阻塞在 read 操作，造成线程资源浪费</p>
</li>
</ul>
<h3 id="_3、reactor模式" tabindex="-1"><a class="header-anchor" href="#_3、reactor模式"><span>3、Reactor模式</span></a></h3>
<ul>
<li>基于 I/O 复用模型：多个连接共用一个阻塞对象，应用程序只需要在一个阻塞对象等待，无需阻塞等待所有连接。当某个连接有新的数据可以处理时，操作系统通知应用程序，线程从阻塞状态返回，开始进行业务处理，Reactor 对应的叫法: 1. 反应器模式 2. 分发者模式(Dispatcher) 3. 通知者模式(notifier)</li>
<li>基于线程池复用线程资源：不必再为每个连接创建线程，将连接完成后的业务处理任务分配给线程进行处理，一个线程可以处理多个连接的业务。</li>
</ul>
<figure><img src="/image/jvm/jvm106.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p><strong>1）说明：</strong></p>
<ul>
<li>
<p>1、Reactor 模式，通过一个或多个输入同时传递给服务处理器的模式(基于事件驱动)</p>
</li>
<li>
<p>2、服务器端程序处理传入的多个请求,并将它们同步分派到相应的处理线程， 因此 Reactor 模式也叫 Dispatcher
模式</p>
</li>
<li>
<p>3、Reactor 模式使用 IO 复用监听事件, 收到事件后，分发给某个线程(进程), 这点就是网络服务器高并发处理关键</p>
</li>
</ul>
...
<li>
<p>2、Handlers：处理程序执行 I/O 事件要完成的实际事件，类似于客户想要与之交谈的公司中的实际官员。Reactor通过调度适当的处理程序来响应 I/O 事件，处理程序执行非阻塞操作。</p>
</li>
</ul>
<p><strong>3）Reactor 模式分类：</strong>
根据 Reactor 的数量和处理资源池线程的数量不同，有 3 种典型的实现</p>
<ul>
<li>
<p>1、单 Reactor 单线程</p>
</li>
...
<li>
<p>3、主从 Reactor 多线程</p>
</li>
</ul>
<p><strong>4）Reactor 模式具有如下的优点：</strong></p>
<ul>
<li>
<p>1、响应快，不必为单个同步时间所阻塞，虽然 Reactor 本身依然是同步的</p>
</li>
<li>
<p>2、可以最大程度的避免复杂的多线程及同步问题，并且避免了多线程/进程的切换开销</p>
</li>
<li>
<p>3、扩展性好，可以方便的通过增加 Reactor 实例个数来充分利用 CPU 资源</p>
</li>
<li>
<p>4、复用性好，Reactor 模型本身与具体事件处理逻辑无关，具有很高的复用性</p>
</li>
</ul>
<h3 id="_4、netty模型" tabindex="-1"><a class="header-anchor" href="#_4、netty模型"><span>4、Netty模型</span></a></h3>
<figure><img src="/image/jvm/jvm107.png" alt="Netty Reactor 工作架构图" tabindex="0" loading="lazy"><figcaption>Netty Reactor 工作架构图</figcaption></figure>
<ul>
<li>
<p>1、Netty 抽象出两组线程池 BossGroup 专门负责接收客户端的连接, WorkerGroup 专门负责网络的读写</p>
</li>
<li>
...
<li>
<p>4、NioEventLoop 表示一个不断循环的执行处理任务的线程， 每个 NioEventLoop 都有一个 selector , 用于监听绑定在其上的 socket 的网络通讯</p>
</li>
<li>
<p>5、NioEventLoopGroup 可以有多个线程, 即可以含有多个 NioEventLoop</p>
</li>
<li>
<p>6、每个 Boss NioEventLoop 循环执行的步骤有 3 步</p>
<ul>
<li>（1）轮询 accept 事件</li>
<li>（2）处理 accept 事件 , 与 client 建立连接 , 生成 NioScocketChannel , 并将其注册到某个 worker NIOEventLoop 上的 selector</li>
<li>（3）处理任务队列的任务 ， 即 runAllTasks</li>
</ul>
</li>
<li>
...
<li>（2）处理 i/o 事件， 即 read , write 事件，在对应 NioScocketChannel 处理</li>
<li>（3）处理任务队列的任务 ， 即 runAllTasks</li>
</ul>
</li>
<li>
<p>8、每个 Worker NIOEventLoop 处理业务时，会使用 pipeline(管道), pipeline 中包含了 channel , 即通过 pipeline可以获取到对应通道, 管道中维护了很多的 处理器</p>
</li>
</ul>
<h3 id="_5、使用示例" tabindex="-1"><a class="header-anchor" href="#_5、使用示例"><span>5、使用示例</span></a></h3>
<h4 id="_1、server" tabindex="-1"><a class="header-anchor" href="#_1、server"><span>1、Server</span></a></h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_6、future-listener-机制" tabindex="-1"><a class="header-anchor" href="#_6、future-listener-机制"><span>6、Future-Listener 机制</span></a></h3>
<ul>
<li>
...
<p>常见有如下操作</p>
<ul>
<li>1、通过 <code v-pre>isDone</code> 方法来判断当前操作是否完成；</li>
<li>2、通过 <code v-pre>isSuccess</code> 方法来判断已完成的当前操作是否成功；</li>
<li>3、通过 <code v-pre>getCause</code> 方法来获取已完成的当前操作失败的原因；</li>
<li>4、通过 <code v-pre>isCancelled</code> 方法来判断已完成的当前操作是否被取消；</li>
<li>5、通过 <code v-pre>addListener </code>方法来注册监听器，当操作已完成(isDone 方法返回完成)，将会通知指定的监听器；</li>
<li>6、如果<code v-pre>Future </code>对象已完成，则通知指定的监听器</li>
</ul>
</li>
</ul>
<h2 id="三、netty核心模块组件" tabindex="-1"><a class="header-anchor" href="#三、netty核心模块组件"><span>三、Netty核心模块组件</span></a></h2>
<h3 id="_1、bootstrap、serverbootstrap" tabindex="-1"><a class="header-anchor" href="#_1、bootstrap、serverbootstrap"><span>1、Bootstrap、ServerBootstrap</span></a></h3>
...
<ul>
<li>
<p><code v-pre>public ServerBootstrap group(EventLoopGroup parentGroup, EventLoopGroup childGroup)</code>，该方法用于服务器端，用来设置两个 EventLoop</p>
</li>
<li>
<p><code v-pre>public B group(EventLoopGroup group) </code>，该方法用于客户端，用来设置一个 EventLoop</p>
</li>
<li>
<p><code v-pre>public B channel(Class&lt;? extends C&gt; channelClass)</code>，该方法用来设置一个服务器端的通道实现</p>
</li>
<li>
<p><code v-pre>public &lt;T&gt; B option(ChannelOption&lt;T&gt; option, T value)</code>，用来给 ServerChannel 添加配置</p>
</li>
<li>
<p><code v-pre>public &lt;T&gt; ServerBootstrap childOption(ChannelOption&lt;T&gt; childOption, T value)</code>，用来给接收到的通道添加配置</p>
</li>
<li>
<p><code v-pre>public ServerBootstrap childHandler(ChannelHandler childHandler) </code>， 该 方 法 用 来 设 置业 务 处 理 类 （ 自 定 义 的handler）</p>
</li>
<li>
<p><code v-pre>public ChannelFuture bind(int inetPort)</code> ，该方法用于服务器端，用来设置占用的端口号</p>
</li>
<li>
<p><code v-pre>public ChannelFuture connect(String inetHost, int inetPort)</code> ，该方法用于客户端，用来连接服务器端</p>
</li>
</ul>
<h3 id="_2、future、channelfuture" tabindex="-1"><a class="header-anchor" href="#_2、future、channelfuture"><span>2、Future、ChannelFuture</span></a></h3>
...
<p>常见的方法有：</p>
<ul>
<li><code v-pre>Channel channel()</code>，返回当前正在进行 IO 操作的通道</li>
<li><code v-pre>ChannelFuture sync()</code>，等待异步操作执行完毕</li>
</ul>
<h3 id="_3、channel" tabindex="-1"><a class="header-anchor" href="#_3、channel"><span>3、Channel</span></a></h3>
<ul>
<li>1、Netty 网络通信的组件，能够用于执行网络 I/O 操作。</li>
<li>2、通过 Channel 可获得当前网络连接的通道的状态</li>
<li>3、通过 Channel 可获得 网络连接的配置参数 （例如接收缓冲区大小）</li>
<li>4、Channel 提供异步的网络 I/O 操作(如建立连接，读写，绑定端口)，异步调用意味着任何 I/O 调用都将立即返回，并且不保证在调用结束时所请求的 I/O 操作已完成</li>
<li>5、调用立即返回一个 ChannelFuture 实例，通过注册监听器到 ChannelFuture 上，可以 I/O 操作成功、失败或取消时回调通知调用方</li>
<li>6、支持关联 I/O 操作与对应的处理程序</li>
<li>7、不同协议、不同的阻塞类型的连接都有不同的 Channel 类型与之对应，常用的 Channel 类型:
<ul>
...
<h3 id="_4、selector" tabindex="-1"><a class="header-anchor" href="#_4、selector"><span>4、Selector</span></a></h3>
<ul>
<li>1、Netty 基于 Selector 对象实现 I/O 多路复用，通过 Selector 一个线程可以监听多个连接的 Channel 事件。</li>
<li>2、当向一个 Selector 中注册 Channel 后，Selector 内部的机制就可以自动不断地查询(Select) 这些注册的Channel 是否有已就绪的 I/O 事件（例如可读，可写，网络连接完成等），这样程序就可以很简单地使用一个线程高效地管理多个 Channel</li>
</ul>
<h3 id="_5、channelhandler-及其实现类" tabindex="-1"><a class="header-anchor" href="#_5、channelhandler-及其实现类"><span>5、ChannelHandler 及其实现类</span></a></h3>
<ul>
<li>
<p>1、ChannelHandler 是一个接口，处理 I/O 事件或拦截 I/O 操作，并将其转发到其 ChannelPipeline(业务处理链)中的下一个处理程序。</p>
</li>
<li>
<p>2、ChannelHandler 本身并没有提供很多方法，因为这个接口有许多的方法需要实现，方便使用期间，可以继承它的子类</p>
</li>
<li>
<p>3、ChannelHandler 及其实现类一览图</p>
</li>
<li>
<p>4、我们经常需要自定义一个 Handler 类去继承 ChannelInboundHandlerAdapter，然后通过重写相应方法实现业务逻辑</p>
<ul>
<li><code v-pre>ChannelActive</code> 通道就绪</li>
<li><code v-pre>ChannelRead</code> 通道读取事件</li>
</ul>
</li>
</ul>
<figure><img src="/image/jvm/jvm108.png" alt="Channel 相关接口一览图" tabindex="0" loading="lazy"><figcaption>Channel 相关接口一览图</figcaption></figure>
<h3 id="_6、pipeline-和-channelpipeline" tabindex="-1"><a class="header-anchor" href="#_6、pipeline-和-channelpipeline"><span>6、Pipeline 和 ChannelPipeline</span></a></h3>
<p>ChannelPipeline 是一个重点：</p>
<ul>
<li>
<p>1、ChannelPipeline 是一个 Handler 的集合，它负责处理和拦截 inbound 或者 outbound 的事件和操作，相当于一个贯穿 Netty 的链。(也可以这样理解：ChannelPipeline 是 保存 ChannelHandler 的 List，用于处理或拦截Channel 的入站事件和出站操作)</p>
</li>
<li>
<p>2、ChannelPipeline 实现了一种高级形式的拦截过滤器模式，使用户可以完全控制事件的处理方式，以及 Channel中各个的 ChannelHandler 如何相互交互</p>
</li>
<li>
<p>3、在 Netty 中每个 Channel 都有且仅有一个 ChannelPipeline 与之对应，它们的组成关系如下</p>
</li>
</ul>
<figure><img src="/image/jvm/jvm109.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<ul>
<li>一个 Channel 包含了一个 ChannelPipeline，而 ChannelPipeline 中又维护了一个由 ChannelHandlerContext组成的双向链表，并且每个 ChannelHandlerContext 中又关联着一个 ChannelHandler</li>
<li>入站事件和出站事件在一个双向链表中，入站事件会从链表 head往后传递到最后一个入站的 handler，出站事件会从链表tail 往前传递到最前一个出站的handler，两种类型的handler 互不干扰</li>
</ul>
<h3 id="_7、channelhandlercontext" tabindex="-1"><a class="header-anchor" href="#_7、channelhandlercontext"><span>7、ChannelHandlerContext</span></a></h3>
<ul>
<li>保存 Channel 相关的所有上下文信息，同时关联一个 ChannelHandler 对象</li>
<li>即 ChannelHandlerContext 中 包 含 一 个 具 体 的 事 件 处 理 器 ChannelHandler ， 同 时ChannelHandlerContext 中也绑定了对应的 pipeline 和 Channel 的信息，方便对 ChannelHandler 进行调用.</li>
</ul>
<h3 id="_8、channeloption" tabindex="-1"><a class="header-anchor" href="#_8、channeloption"><span>8、ChannelOption</span></a></h3>
<p>Netty 在创建 Channel 实例后,一般都需要设置 ChannelOption 参数。</p>
<p>ChannelOption 参数如下:</p>
<ul>
<li>1、<code v-pre>ChannelOption.SO_BACKLOG</code>  对应TCP/IP 协议 listen 函数中的 backlog参数，用来初始化服务器可连接队列大小。服务端处理客户端连接请求是顺序处理的，所以同一时间只能处理一个客户端连接。多个客户端来的时候，服务端将不能处理的客户端连接请求放在队列中等待处理，backlog 参数指定
了队列的大小。</li>
<li>2、<code v-pre>ChannelOption.SO_KEEPALIVE</code>  一直保持连接活动状态</li>
<li>3、<code v-pre>ChannelOption.CONNECT_TIMEOUT_MILLIS</code> 用在客户端建立连接时，如果在指定毫秒内无法连接，会抛出 timeout 异常</li>
<li>4、<code v-pre>ChannelOption.TCP_NODELAY</code></li>
<li>属于 SocketChannal 参数，是 TCP 协议的一个选项，用于是否禁用 Nagle 算法。开启 Nagle 能够减少网络中小分组的数量，提高网络的效率，但实时性较差。关闭 Nagle 可以尽快发出数据包，降低延迟，适合实时游戏、视频通话等需要即时响应的场景。</li>
</ul>
<h3 id="_9、bytebuf" tabindex="-1"><a class="header-anchor" href="#_9、bytebuf"><span>9、ByteBuf</span></a></h3>
<p>ByteBuf 是 Netty 提供的用于处理字节数据的缓冲区类，是 Netty 的核心组件之一。提供了丰富的 API，用于操作字节数据，包括读取、写入、复制、切片等操作。</p>
<p><strong>1）特点</strong></p>
<ul>
<li>灵活的内存分配：ByteBuf 支持自动扩容，并且使用池化技术分配内存，避免频繁地创建和销毁缓冲区，提高了内存利用率</li>
<li>零拷贝：ByteBuf 支持零拷贝技术，可以直接访问 OS 底层数据，避免了数据在应用程序和内核空间之间的复制</li>
<li>读写索引分离：ByteBuf 有独立的读索引和写索引，可以实现零拷贝的同时保持读写操作的独立性</li>
<li>引用计数：ByteBuf 使用引用计数来跟踪缓冲区的引用次数，当引用计数为零时将被自动释放，避免了内存泄漏</li>
<li>可组合和复合：ByteBuf 可以被组合成更大的数据结构，也可以被拆分成多个小的数据结构，支持复杂的数据处理需求</li>
</ul>
<p><strong>2）创建</strong></p>
<p>Netty 创建的 ByteBuf 既可以使用直接内存，也可以使用堆内存，并且默认开启了池化技术。</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">// 默认使用直接内存</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>直接内存 vs 堆内存</strong></p>
<ul>
<li>直接内存创建和销毁的代价昂贵，但读写性能高（少一次内存复制），适合配合池化功能一起用</li>
<li>直接内存对 GC 压力小，因为这部分内存不受 JVM 垃圾回收的管理，但也要注意及时主动释放</li>
</ul>
<p><strong>池化 vs 非池化</strong></p>
<ul>
<li>没有池化，则每次都要创建新的 ByteBuf 实例，这个操作对直接内存代价昂贵，就算是堆内存，也会增加 GC 压力</li>
<li>有了池化，则可以重用池中 ByteBuf 实例，并且采用了与 jemalloc 类似的内存分配算法提升分配效率</li>
<li>高并发时，池化功能更节约内存，减少内存溢出的可能</li>
</ul>
<p>在 Netty 4.1 版本之后，除了 Android 平台默认实现都是开启池化的，可以通过环境变量/VM参数指定是否池化：<code v-pre>-Dio.netty.allocator.type={unpooled|pooled}</code></p>
<p><strong>3）结构</strong></p>
<p>ByteBuf 的内存区域可以分为四个部分，通过若干个索引分割：</p>
<figure><img src="/image/jvm/jvm110.png" alt="ByteBuf结构" tabindex="0" loading="lazy"><figcaption>ByteBuf结构</figcaption></figure>
<p>最开始读写指针都在 0 位置</p>
<ol>
<li>Reader Index 读索引： readerIndex 是一个指针，表示下一个被读取的字节的位置。从 ByteBuf 中读取数据时，readerIndex 会随之移动。可以通过<code v-pre>byteBuf.readerIndex()</code>获取。</li>
<li>Writer Index 写索引： writerIndex 是一个指针，表示下一个可以被写入数据的位置。向 ByteBuf 中写入数据时，writerIndex 会随之移动。可以通过<code v-pre>byteBuf.writerIndex()</code>获取。</li>
<li>Capacity 容量： capacity 表示 ByteBuf 的总容量，即它可以存储的最大字节数。当 writerIndex 达到 capacity 时，ByteBuf 需要重新分配更大的内存来扩容。可以通过<code v-pre>byteBuf.capacity()</code>获取。</li>
<li>Max Capacity 最大容量： maxCapacity 是 ByteBuf 的最大容量，即它能够动态扩容的最大限制。默认是 Integer.MAX_VALUE，可以通过<code v-pre>byteBuf.maxCapacity()</code>获取。</li>
<li>Reference Count 引用计数： ByteBuf 使用引用计数来跟踪缓冲区的引用次数。当引用计数为零时，ByteBuf 内存将被释放。基于 ReferenceCounted 接口实现，可以通过<code v-pre>byteBuf.refCnt()</code>获取</li>
</ol>
<p><strong>4）读写</strong></p>
<p>写方法，写指针会随写入字节数移动；类似的有对应的读方法(write-&gt;read)，读指针随读取字节数移动。</p>
<table>
<thead>
<tr>
<th>方法签名</th>
...
<p>提示</p>
<ul>
<li>未指明返回值的写方法，其返回值都是 this，因此支持链式调用</li>
<li>一系列以 set 开头的写方法可以修改指定位置的数据，不改变写指针位置</li>
<li>一系列以 get 开头的读方法可以读取指定位置的数据，不改变读指针位置</li>
<li>类似 NIO 的 ByteBuffer，<code v-pre>byteBuf.mark[Writer|Reader]Index</code>、<code v-pre>byteBuf.reset[Writer|Reader]Index</code>可以标记、重置读写指针</li>
<li>网络传输，默认习惯是 Big Endian</li>
</ul>
<p><strong>5）扩容机制</strong></p>
<ul>
<li>如果写入后数据大小未超过 4MB，则每次容量翻倍</li>
<li>如果写入后数据大小超过 4MB，则每次增加 <mjx-container v-pre class="MathJax" jax="SVG" style="position: relative;"><svg style="vertical-align: -0.025ex;" xmlns="http://www.w3.org/2000/svg" width="8.72ex" height="1.57ex" role="img" focusable="false" viewBox="0 -683 3854.4 694" aria-hidden="true"><g stroke="currentColor" fill="currentColor" stroke-width="0" transform="scale(1,-1)"><g data-mml-node="math"><g data-mml-node="mn"><path data-c="34" d="M462 0Q444 3 333 3Q217 3 199 0H190V46H221Q241 46 248 46T265 48T279 53T286 61Q287 63 287 115V165H28V211L179 442Q332 674 334 675Q336 677 355 677H373L379 671V211H471V165H379V114Q379 73 379 66T385 54Q393 47 442 46H471V0H462ZM293 211V545L74 212L183 211H293Z"></path></g><g data-mml-node="mi" transform="translate(500,0)"><path data-c="1D440" d="M289 629Q289 635 232 637Q208 637 201 638T194 648Q194 649 196 659Q197 662 198 666T199 671T201 676T203 679T207 681T212 683T220 683T232 684Q238 684 262 684T307 683Q386 683 398 683T414 678Q415 674 451 396L487 117L510 154Q534 190 574 254T662 394Q837 673 839 675Q840 676 842 678T846 681L852 683H948Q965 683 988 683T1017 684Q1051 684 1051 673Q1051 668 1048 656T1045 643Q1041 637 1008 637Q968 636 957 634T939 623Q936 618 867 340T797 59Q797 55 798 54T805 50T822 48T855 46H886Q892 37 892 35Q892 19 885 5Q880 0 869 0Q864 0 828 1T736 2Q675 2 644 2T609 1Q592 1 592 11Q592 13 594 25Q598 41 602 43T625 46Q652 46 685 49Q699 52 704 61Q706 65 742 207T813 490T848 631L654 322Q458 10 453 5Q451 4 449 3Q444 0 433 0Q418 0 415 7Q413 11 374 317L335 624L267 354Q200 88 200 79Q206 46 272 46H282Q288 41 289 37T286 19Q282 3 278 1Q274 0 267 0Q265 0 255 0T221 1T157 2Q127 2 95 1T58 0Q43 0 39 2T35 11Q35 13 38 25T43 40Q45 46 65 46Q135 46 154 86Q158 92 223 354T289 629Z"></path></g><g data-mml-node="mi" transform="translate(1551,0)"><path data-c="1D435" d="M231 637Q204 637 199 638T194 649Q194 676 205 682Q206 683 335 683Q594 683 608 681Q671 671 713 636T756 544Q756 480 698 429T565 360L555 357Q619 348 660 311T702 219Q702 146 630 78T453 1Q446 0 242 0Q42 0 39 2Q35 5 35 10Q35 17 37 24Q42 43 47 45Q51 46 62 46H68Q95 46 128 49Q142 52 147 61Q150 65 219 339T288 628Q288 635 231 637ZM649 544Q649 574 634 600T585 634Q578 636 493 637Q473 637 451 637T416 636H403Q388 635 384 626Q382 622 352 506Q352 503 351 500L320 374H401Q482 374 494 376Q554 386 601 434T649 544ZM595 229Q595 273 572 302T512 336Q506 337 429 337Q311 337 310 336Q310 334 293 263T258 122L240 52Q240 48 252 48T333 46Q422 46 429 47Q491 54 543 105T595 229Z"></path></g><g data-mml-node="mo" transform="translate(2532.2,0)"><path data-c="2217" d="M229 286Q216 420 216 436Q216 454 240 464Q241 464 245 464T251 465Q263 464 273 456T283 436Q283 419 277 356T270 286L328 328Q384 369 389 372T399 375Q412 375 423 365T435 338Q435 325 425 315Q420 312 357 282T289 250L355 219L425 184Q434 175 434 161Q434 146 425 136T401 125Q393 125 383 131T328 171L270 213Q283 79 283 63Q283 53 276 44T250 35Q231 35 224 44T216 63Q216 80 222 143T229 213L171 171Q115 130 110 127Q106 124 100 124Q87 124 76 134T64 161Q64 166 64 169T67 175T72 181T81 188T94 195T113 204T138 215T170 230T210 250L74 315Q65 324 65 338Q65 353 74 363T98 374Q106 374 116 368T171 328L229 286Z"></path></g><g data-mml-node="mi" transform="translate(3254.4,0)"><path data-c="1D45B" d="M21 287Q22 293 24 303T36 341T56 388T89 425T135 442Q171 442 195 424T225 390T231 369Q231 367 232 367L243 378Q304 442 382 442Q436 442 469 415T503 336T465 179T427 52Q427 26 444 26Q450 26 453 27Q482 32 505 65T540 145Q542 153 560 153Q580 153 580 145Q580 144 576 130Q568 101 554 73T508 17T439 -10Q392 -10 371 17T350 73Q350 92 386 193T423 345Q423 404 379 404H374Q288 404 229 303L222 291L189 157Q156 26 151 16Q138 -11 108 -11Q95 -11 87 -5T76 7T74 17Q74 30 112 180T152 343Q153 348 153 366Q153 405 129 405Q91 405 66 305Q60 285 60 284Q58 278 41 278H27Q21 284 21 287Z"></path></g></g></g></svg><mjx-assistive-mml unselectable="on" display="inline"><math xmlns="http://www.w3.org/1998/Math/MathML"><mn>4</mn><mi>M</mi><mi>B</mi><mo>∗</mo><mi>n</mi></math></mjx-assistive-mml></mjx-container>，<mjx-container v-pre class="MathJax" jax="SVG" style="position: relative;"><svg style="vertical-align: -0.025ex;" xmlns="http://www.w3.org/2000/svg" width="1.357ex" height="1.025ex" role="img" focusable="false" viewBox="0 -442 600 453" aria-hidden="true"><g stroke="currentColor" fill="currentColor" stroke-width="0" transform="scale(1,-1)"><g data-mml-node="math"><g data-mml-node="mi"><path data-c="1D45B" d="M21 287Q22 293 24 303T36 341T56 388T89 425T135 442Q171 442 195 424T225 390T231 369Q231 367 232 367L243 378Q304 442 382 442Q436 442 469 415T503 336T465 179T427 52Q427 26 444 26Q450 26 453 27Q482 32 505 65T540 145Q542 153 560 153Q580 153 580 145Q580 144 576 130Q568 101 554 73T508 17T439 -10Q392 -10 371 17T350 73Q350 92 386 193T423 345Q423 404 379 404H374Q288 404 229 303L222 291L189 157Q156 26 151 16Q138 -11 108 -11Q95 -11 87 -5T76 7T74 17Q74 30 112 180T152 343Q153 348 153 366Q153 405 129 405Q91 405 66 305Q60 285 60 284Q58 278 41 278H27Q21 284 21 287Z"></path></g></g></g></svg><mjx-assistive-mml unselectable="on" display="inline"><math xmlns="http://www.w3.org/1998/Math/MathML"><mi>n</mi></math></mjx-assistive-mml></mjx-container> 取决于容量足够即可，目的是避免内存浪费</li>
<li>扩容的大小不能超过 max capacity</li>
</ul>
<p><strong>6）内存回收</strong></p>
<p>基于堆外内存的 ByteBuf 实现最好是手动来释放，而不是等 GC 垃圾回收。</p>
<ul>
<li>UnpooledHeapByteBuf 使用 JVM 内存，只需等 GC 回收内存即可</li>
<li>UnpooledDirectByteBuf 使用直接内存，需要特殊的方法来回收内存</li>
<li>PooledByteBuf 及其子类使用了池化机制，需要更复杂的规则来回收内存</li>
</ul>
<p>回收内存的源码实现，可以关注 <code v-pre>protected abstract void deallocate()</code>方法的不同实现。</p>
<p>原理上，Netty 采用了引用计数法来控制回收内存，每种 ByteBuf 都实现了 ReferenceCounted 接口：</p>
<ul>
<li>每个 ByteBuf 对象的初始计数为 1</li>
<li>调用 <code v-pre>byteBuf.release()</code> 方法计数减 1</li>
<li>调用 <code v-pre>byteBuf.retain()</code> 方法计数加 1</li>
<li>当计数为 0 时，底层内存会被回收，这时即使 ByteBuf 对象还在，其各个方法均无法正常使用</li>
</ul>
<p><strong>释放规则</strong></p>
...
<p><strong>7）零拷贝</strong></p>
<p>和 OS 里面的零拷贝意义略有不同，Netty 的零拷贝指的是对 ByteBuf 的复制、拼接在底层没有发生内存的拷贝，还是使用原来的那块内存区域，只不过新增了一些指针来维护不同的实例。</p>
<p><strong>切片</strong></p>
<p><code v-pre>byteBuf.slice()</code>对 ByteBuf 进行切片，并不发生内存复制，切片后的多个 ByteBuf 各自维护独立的 read，write 指针。</p>
<figure><img src="/image/jvm/jvm111.png" alt="ByteBuf切片" tabindex="0" loading="lazy"><figcaption>ByteBuf切片</figcaption></figure>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>复制</strong></p>
<p><code v-pre>byteBuf.duplicate()</code>截取原始 ByteBuf 的所有内容，不发生内存拷贝，仅新增维护指针。</p>
<figure><img src="/image/jvm/jvm112.png" alt="ByteBuf复制" tabindex="0" loading="lazy"><figcaption>ByteBuf复制</figcaption></figure>
<p><code v-pre>byteBuf.copy()</code>则是执行了深拷贝，无论读写都和原 ByteBuf 无关。</p>
<p><strong>组合</strong></p>
<p>CompositeByteBuf 是一个组合的 ByteBuf，内部维护了一个 Component 数组，每个 Component 管理一个 ByteBuf，记录了这个 ByteBuf 相对于整体偏移量的信息，代表着整体中某一段的数据。</p>
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>8）Unpooled</strong></p>
<p>Unpooled 是一个工具类，提供了非池化的 ByteBuf 创建、组合、复制等操作，例如<code v-pre>wrappedBuffer</code>用于包装 ByteBuf，并且底层不会发生拷贝。</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">// 当包装 ByteBuf 个数超过一个时, 底层使用了 CompositeByteBuf</span>
