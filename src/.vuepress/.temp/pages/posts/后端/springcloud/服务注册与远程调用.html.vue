<template><div><!--more--->
<h1 id="服务注册与远程调用" tabindex="-1"><a class="header-anchor" href="#服务注册与远程调用"><span>服务注册与远程调用</span></a></h1>
<h2 id="一、介绍" tabindex="-1"><a class="header-anchor" href="#一、介绍"><span>一、介绍</span></a></h2>
<p>在拆分黑马商城的时候，我们发现一个问题：就是购物车业务中需要查询商品信息，但商品信息查询的逻辑全部迁移到了<code v-pre>item-service</code>服务，导致我们无法查询。</p>
...
<p>那么问题来了：我们该如何跨服务调用，准确的说，如何在<code v-pre>cart-service</code>中获取<code v-pre>item-service</code>服务中的提供的商品数据呢？</p>
<p>大家思考一下，我们以前有没有实现过类似的远程查询的功能呢？</p>
<p>答案是肯定的，我们前端向服务端查询数据，其实就是从浏览器远程查询服务端数据。比如我们刚才通过Swagger测试商品查询接口，就是向<code v-pre>http://localhost:8081/items</code>这个接口发起的请求：</p>
<h3 id="_1、resttemplate" tabindex="-1"><a class="header-anchor" href="#_1、resttemplate"><span>1、RestTemplate</span></a></h3>
<p>Spring给我们提供了一个RestTemplate的API，可以方便的实现Http请求的发送。</p>
<div class="hint-container info">
<p class="hint-container-title">RestTemplate使用的基本步骤如下</p>
<ul>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">RemoteCallConfig</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2、远程调用" tabindex="-1"><a class="header-anchor" href="#_2、远程调用"><span>2、远程调用</span></a></h3>
<p>接下来，我们修改<code v-pre>cart-service</code>中的<code v-pre>com.hmall.cart.service.impl.``CartServiceImpl</code>的<code v-pre>handleCartItems</code>方法，发送http请求到<code v-pre>item-service</code>：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>        <span class="token comment">// List&lt;ItemDTO> items = itemService.queryItemByIds(itemIds);</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>可以看到，利用RestTemplate发送http请求与前端ajax发送请求非常相似，都包含四部分信息：</p>
<ul>
<li>
<p>请求方式</p>
</li>
...
<h3 id="_3、缺点" tabindex="-1"><a class="header-anchor" href="#_3、缺点"><span>3、缺点</span></a></h3>
<p>假如商品微服务被调用较多，为了应对更高的并发，我们进行了多实例部署此时，每个<code v-pre>item-service</code>的实例其IP或端口不同，问题来了：</p>
<ul>
<li>item-service这么多实例，cart-service如何知道每一个实例的地址？</li>
<li>http请求要写url地址，<code v-pre>cart-service</code>服务到底该调用哪个实例呢？</li>
<li>如果在运行过程中，某一个<code v-pre>item-service</code>实例宕机，<code v-pre>cart-service</code>依然在调用该怎么办？</li>
<li>如果并发太高，<code v-pre>item-service</code>临时多部署了N台实例，<code v-pre>cart-service</code>如何知道新实例的地址？</li>
</ul>
<p>为了解决上述问题，就必须引入注册中心的概念了</p>
<h2 id="二、服务注册-nacos" tabindex="-1"><a class="header-anchor" href="#二、服务注册-nacos"><span>二、服务注册(Nacos)</span></a></h2>
<h3 id="_1、注册中心原理" tabindex="-1"><a class="header-anchor" href="#_1、注册中心原理"><span>1、注册中心原理</span></a></h3>
<p>在微服务远程调用的过程中，包括两个角色：</p>
<ul>
<li>服务提供者：提供接口供其它微服务访问，比如<code v-pre>item-service</code></li>
<li>服务消费者：调用其它微服务提供的接口，比如<code v-pre>cart-service</code></li>
</ul>
<p>在大型微服务项目中，服务提供者的数量会非常多，为了管理这些服务就引入了<strong>注册中心</strong>的概念。注册中心、服务提供者、服务消费者三者间关系如下：</p>
<figure><img src="/image/cloud/cloud1.jpg" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<div class="hint-container tip">
<p class="hint-container-title">流程如下</p>
<ol>
<li>
<p>服务启动时就会注册自己的服务信息（服务名、IP、端口）到注册中心</p>
</li>
<li>
<p>调用者可以从注册中心订阅想要的服务，获取服务对应的实例列表（1个服务可能多实例部署）</p>
</li>
<li>
<p>调用者自己对实例列表负载均衡，挑选一个实例</p>
</li>
<li>
<p>调用者向该实例发起远程调用</p>
</li>
...
<h3 id="_2、nacos注册中心" tabindex="-1"><a class="header-anchor" href="#_2、nacos注册中心"><span>2、Nacos注册中心</span></a></h3>
<div class="hint-container info">
<p class="hint-container-title">目前开源的注册中心框架有很多，国内比较常见的有</p>
<ul>
...
<p><strong>1）配置MySQL</strong></p>
<p>我们基于Docker来部署Nacos的注册中心，首先我们要准备MySQL数据库表，用来存储Nacos的数据,mysql脚本可以从github下载</p>
<p><a href="https://github.com/alibaba/nacos/tree/develop/distribution/conf" target="_blank" rel="noopener noreferrer">下载地址</a></p>
<figure><img src="/image/cloud/cloud1.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p><strong>2）修改配置文件</strong></p>
<p>创建一个<code v-pre>nacos/custom.env</code>文件，MYSQL_SERVICE_HOST也就是mysql地址，需要修改为你自己的虚拟机IP地址：</p>
<div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre v-pre class="language-text"><code>PREFER_HOST_MODE=hostname
MODE=standalone
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>3）创建镜像</strong></p>
<p>然后，将刚刚创建的的<code v-pre>nacos</code>目录上传至虚拟机的<code v-pre>/root</code>目录。进入root目录，然后执行下面的docker命令：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>启动完成后，访问下面地址：http://192.168.175.129:8848/nacos/，注意将<code v-pre>192.168.175.129</code>替换为你自己的虚拟机IP地址。</p>
...
<p>接下来，我们把<code v-pre>item-service</code>注册到Nacos</p>
<div class="hint-container info">
<p class="hint-container-title">步骤如下：</p>
<ul>
<li>引入依赖</li>
<li>配置Nacos地址</li>
<li>重启</li>
...
<p>在<code v-pre>item-service</code>的<code v-pre>pom.xml</code>中添加依赖：</p>
<div class="language-xml line-numbers-mode" data-ext="xml" data-title="xml"><pre v-pre class="language-xml"><code><span class="token comment">&lt;!--nacos 服务注册发现--></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）配置Nacos</strong></p>
<p>在<code v-pre>item-service</code>的<code v-pre>application.yml</code>中添加nacos地址配置：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>3）启动</strong></p>
<p>然后配置启动项，注意重命名并且配置新的端口，避免冲突。</p>
<p><code v-pre>VM options:   -Dserver.port=8083</code></p>
<p>访问nacos控制台，可以发现服务注册成功。</p>
<h3 id="_4、服务发现" tabindex="-1"><a class="header-anchor" href="#_4、服务发现"><span>4、服务发现</span></a></h3>
<p>服务的消费者要去nacos订阅服务，这个过程就是服务发现</p>
<div class="hint-container info">
<p class="hint-container-title">步骤如下：</p>
<ul>
<li>引入依赖</li>
...
<p>我们在<code v-pre>cart-service</code>中的<code v-pre>pom.xml</code>中添加下面的依赖：</p>
<div class="language-xml line-numbers-mode" data-ext="xml" data-title="xml"><pre v-pre class="language-xml"><code><span class="token comment">&lt;!--nacos 服务注册发现--></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependency</span><span class="token punctuation">></span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>groupId</span><span class="token punctuation">></span></span>com.alibaba.cloud<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>groupId</span><span class="token punctuation">></span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>artifactId</span><span class="token punctuation">></span></span>spring-cloud-starter-alibaba-nacos-discovery<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>artifactId</span><span class="token punctuation">></span></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）配置Nacos地址</strong></p>
<p>在<code v-pre>cart-service</code>的<code v-pre>application.yml</code>中添加nacos地址配置：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">spring</span><span class="token punctuation">:</span>
  <span class="token key atrule">cloud</span><span class="token punctuation">:</span>
    <span class="token key atrule">nacos</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>3）发现并调用服务</strong></p>
<p>接下来，服务调用者<code v-pre>cart-service</code>就可以去订阅<code v-pre>item-service</code>服务了。不过item-service有多个实例，而真正发起调用时只需要知道一个实例的地址。</p>
...
<p>另外，服务发现需要用到一个工具，<code v-pre>DiscoveryClient</code>，SpringCloud已经帮我们自动装配，我们可以直接注入使用：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>        <span class="token comment">//2.1、根据服务名称获取服务列表</span>
        ...
        <span class="token punctuation">}</span>
        <span class="token class-name">List</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">ItemDTO</span><span class="token punctuation">></span></span> items <span class="token operator">=</span> response<span class="token punctuation">.</span><span class="token function">getBody</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="三、远程调用-openfeign" tabindex="-1"><a class="header-anchor" href="#三、远程调用-openfeign"><span>三、远程调用(OpenFeign)</span></a></h2>
<p>​		我们利用Nacos实现了服务的治理，利用RestTemplate实现了服务的远程调用。但是远程调用的代码太复杂了：而且这种调用方式，与原本的本地方法调用差异太大，编程时的体验也不统一，一会儿远程调用，一会儿本地调用。因此，我们必须想办法改变远程调用的开发模式，让<strong>远程调用像本地方法调用一样简单</strong>。而这就要用到<strong>OpenFeign</strong>组件了。</p>
<div class="hint-container tip">
<p class="hint-container-title">其实远程调用的关键点就在于四个：</p>
<ul>
<li>请求方式</li>
<li>请求路径</li>
<li>请求参数</li>
...
<p><strong>1）引入依赖</strong></p>
<p>在<code v-pre>cart-service</code>服务的pom.xml中引入<code v-pre>OpenFeign</code>的依赖和<code v-pre>loadBalancer</code>依赖：</p>
<div class="language-xml line-numbers-mode" data-ext="xml" data-title="xml"><pre v-pre class="language-xml"><code>  <span class="token comment">&lt;!--openFeign--></span>
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependency</span><span class="token punctuation">></span></span>
  ...
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）启用OpenFeign</strong></p>
<p>接下来，我们在<code v-pre>cart-service</code>的<code v-pre>CartApplication</code>启动类上添加注解，启动OpenFeign功能：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@EnableFeignClients</span>
...
<p>在<code v-pre>cart-service</code>中，定义一个新的接口，编写Feign客户端：</p>
<p>其中代码如下：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里只需要声明接口，无需实现方法</p>
<div class="hint-container tip">
<p class="hint-container-title">接口中的几个关键信息</p>
<ul>
<li><code v-pre>@FeignClient(&quot;item-service&quot;)</code> ：声明服务名称</li>
<li><code v-pre>@GetMapping</code> ：声明请求方式</li>
<li><code v-pre>@GetMapping(&quot;/items&quot;)</code> ：声明请求路径</li>
<li><code v-pre>@RequestParam(&quot;ids&quot;) Collection&lt;Long&gt; ids</code> ：声明请求参数</li>
<li><code v-pre>List&lt;ItemDTO&gt;</code> ：返回值类型</li>
</ul>
</div>
<p><strong>4）使用FeignClient</strong></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>feign替我们完成了服务拉取、负载均衡、发送http请求的所有工作。</p>
<h3 id="_2、连接池" tabindex="-1"><a class="header-anchor" href="#_2、连接池"><span>2、连接池</span></a></h3>
<p>Feign底层发起http请求，依赖于其它的框架。</p>
<div class="hint-container info">
<p class="hint-container-title">其底层支持的http客户端实现包括</p>
<ul>
...
<p><strong>1）引入依赖</strong></p>
<p>在<code v-pre>cart-service</code>的<code v-pre>pom.xml</code>中引入依赖：</p>
<div class="language-xml line-numbers-mode" data-ext="xml" data-title="xml"><pre v-pre class="language-xml"><code><span class="token comment">&lt;!--OK http 的依赖 --></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependency</span><span class="token punctuation">></span></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）开启连接池</strong></p>
<p>在<code v-pre>cart-service</code>的<code v-pre>application.yml</code>配置文件中开启Feign的连接池功能：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">feign</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>重启服务，连接池就生效了。</p>
<h3 id="_3、结构优化" tabindex="-1"><a class="header-anchor" href="#_3、结构优化"><span>3、结构优化</span></a></h3>
<p>如果拆分了交易微服务（<code v-pre>trade-service</code>），它也需要远程调用<code v-pre>item-service</code>中的根据id批量查询商品功能。这个需求与<code v-pre>cart-service</code>中是一样的。因此，我们就需要在<code v-pre>trade-service</code>中再次定义<code v-pre>ItemClient</code>接口，这不是重复编码吗？ 有什么办法能加避免重复编码呢？</p>
<h4 id="_1-思路分析" tabindex="-1"><a class="header-anchor" href="#_1-思路分析"><span>1）思路分析</span></a></h4>
<p>避免重复编码的办法就是<strong>抽取</strong>。不过这里有两种抽取思路：</p>
<ul>
<li>思路1：抽取到微服务之外的公共module</li>
<li>思路2：每个微服务自己抽取一个module</li>
</ul>
<figure><img src="/image/cloud/cloud2.jpg" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>方案1抽取更加简单，工程结构也比较清晰，但缺点是整个项目耦合度偏高。</p>
<p>方案2抽取相对麻烦，工程结构相对更复杂，但服务之间耦合度降低。</p>
<h4 id="_2-抽取feign客户端" tabindex="-1"><a class="header-anchor" href="#_2-抽取feign客户端"><span>2）抽取Feign客户端</span></a></h4>
<p>在<code v-pre>hmall</code>下定义一个新的module，命名为hm-api，然后把ItemDTO和ItemClient都拷贝过来，最终结构如下：</p>
<figure><img src="/image/cloud/cloud2.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>现在，任何微服务要调用<code v-pre>item-service</code>中的接口，只需要引入<code v-pre>hm-api</code>模块依赖即可，无需自己编写Feign客户端了。</p>
<p>接下来，我们在<code v-pre>cart-service</code>的<code v-pre>pom.xml</code>中引入<code v-pre>hm-api</code>模块：</p>
<div class="language-xml line-numbers-mode" data-ext="xml" data-title="xml"><pre v-pre class="language-xml"><code>  <span class="token comment">&lt;!--feign模块--></span>
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependency</span><span class="token punctuation">></span></span>
...
<p class="hint-container-title">启动类上添加声明</p>
<p>在cart-service的启动类上添加声明</p>
<p><code v-pre>@EnableFeignClients(basePackages = &quot;com.hmall.api.client&quot;)</code></p>
<p>或者声明要用的FeignClient</p>
<p><code v-pre>@EnableFeignClients(clients= {ItemClient.class})</code></p>
<p>否则会扫描不到<code v-pre>ItemClient</code>，从而报错</p>
</div>
<h3 id="_4、日志配置" tabindex="-1"><a class="header-anchor" href="#_4、日志配置"><span>4、日志配置</span></a></h3>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DefaultFeignConfig</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）配置</strong></p>
<p>接下来，要让日志级别生效，还需要配置这个类。有两种方式：</p>
<ul>
<li><strong>局部</strong>生效：在某个<code v-pre>FeignClient</code>中配置，只对当前<code v-pre>FeignClient</code>生效</li>
</ul>
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>@FeignClient(value = &quot;item-service&quot;, configuration = DefaultFeignConfig.class)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul>
<li><strong>全局</strong>生效：在<code v-pre>@EnableFeignClients</code>中配置，针对所有<code v-pre>FeignClient</code>生效。</li>
</ul>
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>@EnableFeignClients(defaultConfiguration = DefaultFeignConfig.class)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_5、openfeign-传递用户" tabindex="-1"><a class="header-anchor" href="#_5、openfeign-传递用户"><span>5、OPenFeign 传递用户</span></a></h3>
<p><RouteLink to="/posts/%E5%90%8E%E7%AB%AF/springcloud/%E7%BB%9F%E4%B8%80%E7%BD%91%E5%85%B3%E8%B7%AF%E7%94%B1_4.html">参考网关部分</RouteLink></p>
</div></template>
