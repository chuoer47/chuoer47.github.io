<template><div><!--more--->
<h1 id="分布式事务" tabindex="-1"><a class="header-anchor" href="#分布式事务"><span>分布式事务</span></a></h1>
...
<ul>
<li>参与事务的多个子业务在不同的微服务，跨越了不同的数据库。虽然每个单独的业务都能在本地遵循ACID，但是它们互相之间没有感知，不知道有人失败了，无法保证最终结果的统一，也就无法遵循ACID的事务特性了。这就是分布式事务问题.</li>
</ul>
<div class="hint-container tip">
<p class="hint-container-title">出现以下情况之一就可能产生分布式事务问题：</p>
<ul>
<li>业务跨多个服务实现</li>
<li>业务跨多个数据源实现</li>
</ul>
</div>
<h2 id="二、seata" tabindex="-1"><a class="header-anchor" href="#二、seata"><span>二、Seata</span></a></h2>
<p>解决分布式事务的方案有很多，但实现起来都比较复杂，因此我们一般会使用开源的框架来解决分布式事务问题。在众多的开源分布式事务框架中，功能最完善、使用最多的就是阿里巴巴在2019年开源的Seata了，<a href="https://seata.apache.org/zh-cn/docs/overview/what-is-seata/" target="_blank" rel="noopener noreferrer">官方文档</a></p>
<p>解决分布式事务的思想非常简单：</p>
...
<li><strong>RM (Resource Manager) - 资源管理器：</strong> 管理分支事务，与TC交谈以注册分支事务和报告分支事务的状态，并驱动分支事务提交或回滚。</li>
</ul>
<p>Seata的工作架构如图所示：</p>
<figure><img src="/image/cloud/cloud17.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<ul>
<li>
...
<h2 id="三、部署tc服务" tabindex="-1"><a class="header-anchor" href="#三、部署tc服务"><span>三、部署TC服务</span></a></h2>
<p>使用docker部署seata,可参考<a href="https://seata.apache.org/zh-cn/docs/ops/deploy-by-docker-142/" target="_blank" rel="noopener noreferrer">官网</a></p>
<h3 id="_1、准备数据库表" tabindex="-1"><a class="header-anchor" href="#_1、准备数据库表"><span>1、准备数据库表</span></a></h3>
<p>Seata支持多种存储模式，但考虑到持久化的需要，我们一般选择基于数据库存储。使用<a href="https://github.com/apache/incubator-seata/blob/develop/script/server/db/mysql.sql" target="_blank" rel="noopener noreferrer">MySQL脚本</a>，初始化 Seata TC Server 的 db 数据库。</p>
<h3 id="_2、准备配置文件" tabindex="-1"><a class="header-anchor" href="#_2、准备配置文件"><span>2、准备配置文件</span></a></h3>
<p>准备一个seata目录，其中包含了seata运行时所需要的配置文件：</p>
<figure><img src="/image/cloud/cloud18.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>将整个seata文件夹拷贝到虚拟机的<code v-pre>/root</code>目录.</p>
<h3 id="_3、docker部署" tabindex="-1"><a class="header-anchor" href="#_3、docker部署"><span>3、Docker部署</span></a></h3>
<p>需要注意，要确保nacos、mysql都在hm-net网络中。如果某个容器不再hm-net网络，可以参考下面的命令将某容器加入指定网络：</p>
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> network connect <span class="token punctuation">[</span>网络名<span class="token punctuation">]</span> <span class="token punctuation">[</span>容器名<span class="token punctuation">]</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>在虚拟机的<code v-pre>/root</code>目录执行下面的命令：</p>
<div class="language-Shell line-numbers-mode" data-ext="Shell" data-title="Shell"><pre v-pre class="language-Shell"><code>docker run --name seata \
-p 8099:8099 \
-p 7099:7099 \
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="四、微服务集成seata" tabindex="-1"><a class="header-anchor" href="#四、微服务集成seata"><span>四、微服务集成Seata</span></a></h2>
<p>参与分布式事务的每一个微服务都需要集成Seata，我们以<code v-pre>trade-service</code>为例。</p>
<h3 id="_1、引入依赖" tabindex="-1"><a class="header-anchor" href="#_1、引入依赖"><span>1、引入依赖</span></a></h3>
<p>为了方便各个微服务集成seata，我们需要把seata配置共享到nacos，因此<code v-pre>trade-service</code>模块不仅仅要引入seata依赖，还要引入nacos依赖:</p>
<div class="language-XML line-numbers-mode" data-ext="XML" data-title="XML"><pre v-pre class="language-XML"><code>	&lt;!--统一配置管理--&gt;
  ...
  &lt;/dependency&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2、改造配置" tabindex="-1"><a class="header-anchor" href="#_2、改造配置"><span>2、改造配置</span></a></h3>
<p>首先在nacos上添加一个共享的seata配置，命名为<code v-pre>shared-seata.yaml</code>：</p>
<div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">seata</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后，改造<code v-pre>trade-service</code>模块，添加<code v-pre>bootstrap.yaml</code>：</p>
<p>内容如下:</p>
...
hm:
  ...
  db:
    database: hm-trade
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>参考上述办法分别改造<code v-pre>hm-cart</code>和<code v-pre>hm-item</code>两个微服务模块。</p>
<h3 id="_3、添加数据库表" tabindex="-1"><a class="header-anchor" href="#_3、添加数据库表"><span>3、添加数据库表</span></a></h3>
<p>（AT模式下）seata的客户端在解决分布式事务的时候需要记录一些中间数据，保存在数据库中。因此我们要先准备一个这样的表,<a href="https://github.com/apache/incubator-seata/blob/develop/script/client/at/db/mysql.sql" target="_blank" rel="noopener noreferrer">参考</a>。</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- for AT mode you must to init this sql for you business database. the seata server not need it.</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>至此为止，微服务整合的工作就完成了。</p>
<h2 id="五、xa模式" tabindex="-1"><a class="header-anchor" href="#五、xa模式"><span>五、XA模式</span></a></h2>
...
<p><code v-pre>XA</code> 规范 是<code v-pre> X/Open</code> 组织定义的分布式事务处理（DTP，Distributed Transaction Processing）标准，XA 规范 描述了全局的<code v-pre>TM</code>与局部的<code v-pre>RM</code>之间的接口，几乎所有主流的数据库都对 XA 规范 提供了支持。</p>
<h3 id="_2、seata的xa模型" tabindex="-1"><a class="header-anchor" href="#_2、seata的xa模型"><span>2、Seata的XA模型</span></a></h3>
<p>Seata对原始的XA模式做了简单的封装和改造，以适应自己的事务模型，基本架构如图：</p>
<figure><img src="/image/cloud/cloud19.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p><code v-pre>RM</code>一阶段的工作：</p>
<ol>
<li>注册分支事务到<code v-pre>TC</code></li>
<li>执行分支业务sql但<strong>不提交</strong></li>
<li>报告执行状态到<code v-pre>TC</code></li>
</ol>
<p><code v-pre>TC</code>二阶段的工作：</p>
<ol>
<li><code v-pre>TC</code>检测各分支事务执行状态</li>
<li>如果都成功，通知所有RM提交事务</li>
<li>如果有失败，通知所有RM回滚事务</li>
</ol>
<p><code v-pre>RM</code>二阶段的工作：</p>
<ul>
<li>接收<code v-pre>TC</code>指令，提交或回滚事务</li>
</ul>
<div class="hint-container tip">
<p class="hint-container-title">XA模式优缺点</p>
<ul>
<li>
<p>优点</p>
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>其次，我们要利用<code v-pre>@GlobalTransactional</code>标记分布式事务的入口方法:</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    <span class="token annotation punctuation">@Override</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="六、at模式-默认" tabindex="-1"><a class="header-anchor" href="#六、at模式-默认"><span>六、AT模式(默认)</span></a></h2>
<p><code v-pre>AT</code>模式同样是分阶段提交的事务模型，不过<strong>缺弥补了<code v-pre>XA</code>模型中资源锁定周期过长的缺陷。</strong></p>
<h3 id="_1、seata的at模型" tabindex="-1"><a class="header-anchor" href="#_1、seata的at模型"><span>1、Seata的AT模型</span></a></h3>
<figure><img src="/image/cloud/cloud20.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>阶段一<code v-pre>RM</code>的工作：</p>
<ul>
<li>注册分支事务</li>
...
<p>阶段二提交时<code v-pre>RM</code>的工作：</p>
<ul>
<li>删除undo-log即可</li>
</ul>
<p>阶段二回滚时<code v-pre>RM</code>的工作：</p>
<ul>
<li>根据undo-log恢复数据到更新前</li>
</ul>
<div class="hint-container info">
<p class="hint-container-title">AT与XA的区别</p>
<ul>
<li><code v-pre>XA</code>模式一阶段不提交事务，锁定资源；<code v-pre>AT</code>模式一阶段直接提交，不锁定资源。</li>
<li><code v-pre>XA</code>模式依赖数据库机制实现回滚；<code v-pre>AT</code>模式利用数据快照实现数据回滚。</li>
<li><code v-pre>XA</code>模式强一致；<code v-pre>AT</code>模式最终一致</li>
</ul>
</div>
...
<p>使用AT模型需要为<strong>每个</strong>微服务的数据库添加<code v-pre>undo_log</code>表</p>
</div>
<p>直接在Nacos中的共享shared-seata.yaml配置文件中设置</p>
<div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">seata</span><span class="token punctuation">:</span>
  <span class="token key atrule">data-source-proxy-mode</span><span class="token punctuation">:</span> AT
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>其次，我们和AT一样要利用<code v-pre>@GlobalTransactional</code>标记分布式事务的入口方法。</p>
</div></template>
