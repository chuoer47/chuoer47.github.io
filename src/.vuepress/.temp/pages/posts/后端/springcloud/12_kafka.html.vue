<template><div><!--more--->
<h1 id="kafka详解" tabindex="-1"><a class="header-anchor" href="#kafka详解"><span>Kafka详解</span></a></h1>
<h2 id="一、kafka介绍" tabindex="-1"><a class="header-anchor" href="#一、kafka介绍"><span>一、Kafka介绍</span></a></h2>
...
<figure><img src="/image\spring\spring24.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p><strong>kafka主要包括四个组件</strong></p>
<figure><img src="/image\spring\spring25.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<ul>
<li><strong>1)Producer(生产者)</strong>:负责将数据发布到 Kafka 的特定,Topic上。它会根据要求将数据以不同的分区策略分布到各个分区里</li>
<li><strong>2)Consumer(消费者)</strong>:从Kafka的 Topic 中读取数据。消费者可以属于某个消费组(Consumer Group)，这样可以让多个消费者平衡负载读取数据。</li>
<li><strong>3)Broker(消息代理)</strong>:是Kafka的核心，消息在这里存储和管理。每个Kafka集群可以包含一个或多个 Broker，负责接收、存储、以及发送数据。</li>
<li><strong>4)Zookeeper(协调器)</strong>:用于Kafka 的分布式协调和管理任务，比如存储 Broker 的元数据信息、分区列表、Leader 等等。Zookeeper 确保 Kafka 集群的高可用性和一致性。</li>
</ul>
<h3 id="_2、kafka安装配置" tabindex="-1"><a class="header-anchor" href="#_2、kafka安装配置"><span>2、kafka安装配置</span></a></h3>
<p>Kafka对于zookeeper是强依赖，保存kafka相关的节点数据，所以安装Kafka之前必须先安装zookeeper</p>
<ul>
<li>Docker安装zookeeper</li>
</ul>
<p>下载镜像：</p>
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> pull zookeeper:3.4.14
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>创建容器</p>
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">-d</span> <span class="token parameter variable">--name</span> zookeeper <span class="token parameter variable">-p</span> <span class="token number">2181</span>:2181 <span class="token parameter variable">--network</span> hm-net zookeeper:3.4.14
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul>
<li>Docker安装kafka</li>
</ul>
<p>下载镜像：</p>
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> pull wurstmeister/kafka:2.12-2.3.1
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>创建容器</p>
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token function">docker</span> run <span class="token parameter variable">-d</span> <span class="token parameter variable">--name</span> kafka <span class="token punctuation">\</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3、kafka入门" tabindex="-1"><a class="header-anchor" href="#_3、kafka入门"><span>3、kafka入门</span></a></h3>
<figure><img src="/image\spring\spring26.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<ul>
...
<ul>
<li>生产者发送消息，多个消费者（同一个消费者组）订阅同一个主题，只能有一个消费者收到消息（一对一）</li>
<li>生产者发送消息，多个消费者（不同消费者组）订阅同一个主题，所有消费者都能收到消息（一对多）</li>
</ul>
<h2 id="二、kafka高可用设计" tabindex="-1"><a class="header-anchor" href="#二、kafka高可用设计"><span>二、kafka高可用设计</span></a></h2>
<h3 id="_1、集群" tabindex="-1"><a class="header-anchor" href="#_1、集群"><span>1、集群</span></a></h3>
<figure><img src="/image\spring\spring27.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<ul>
<li>
...
<h3 id="_2、备份机制-replication" tabindex="-1"><a class="header-anchor" href="#_2、备份机制-replication"><span>2、备份机制(Replication）</span></a></h3>
<figure><img src="/image\spring\spring28.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>Kafka 中消息的备份又叫做 副本（Replica）</p>
<div class="hint-container info">
<p class="hint-container-title">Kafka 定义了两类副本：</p>
<ul>
<li>
...
<figure><img src="/image\spring\spring29.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p><strong>ISR（in-sync replica）</strong> 需要同步复制保存的follower</p>
<ul>
<li>
<p>如果leader失效后，需要选出新的leader，选举的原则如下：</p>
<ul>
<li>第一：选举时优先从ISR中选定，因为这个列表中follower的数据是与leader同步的</li>
<li>第二：如果ISR列表中的follower都不行了，就只能从其他follower中选取</li>
</ul>
</li>
<li>
<p>极端情况，就是所有副本都失效了，这时有两种方案</p>
<ul>
<li>第一：等待ISR中的一个活过来，选为Leader，数据可靠，但活过来的时间不确定</li>
<li>第二：选择第一个活过来的Replication，不一定是ISR中的，选为leader，以最快速度恢复可用性，但数据不一定完整</li>
</ul>
</li>
</ul>
<h3 id="_3、ack机制" tabindex="-1"><a class="header-anchor" href="#_3、ack机制"><span>3、ACK机制</span></a></h3>
<p>在生产者发送消息时，可以配置不同的确认级别(acks)，例如 acks=all 则需要等待所有 ISR 中的副本确认收到消息，进一步提高可靠性。</p>
<table>
<thead>
<tr>
...
<h3 id="_4、其他" tabindex="-1"><a class="header-anchor" href="#_4、其他"><span>4、其他</span></a></h3>
<p><strong>1)ZooKeeper:</strong>  Kafka 通过 ZooKeeper 来管理集群的元数据和协调节点之间的工作。比如，分区 Leader 或者 Follower 的选举等操作都是靠 ZooKeeper 来完成的。这样即便某个 Kafka Broker 挂掉了，ZooKeeper 也能迅速协调恢复。</p>
<p><strong>2)高效的存储格式:</strong> Kafka的数据存储采用了顺序写入的方式，而非像传统数据库那样频繁的读写操作跳跃性强。顺序写入拥有很高的磁盘写入速度，极大地提升了 Kafka 的性能。</p>
<p><strong>3)Segment 和Index 文件:</strong> Kafka对每个 Partition 生成多个 Segment 文件和索引文件。Segment 文件是实际存储消息的，而索引文件则是维护消息偏移量和物理位置对照表。这样一来，即使是非常大的数据量，Kafka 也能效地搜索和读取消息。</p>
<p><strong>4)Min lSR 机制:</strong> 配置 min.insync.replicas 参数可以设定 ISR值，当 ISR 数量低于这个阈值时，Kafka 会拒绝消息写入请求，以确保数据的足够兄余。</p>
<h2 id="三、kafka生产者详解" tabindex="-1"><a class="header-anchor" href="#三、kafka生产者详解"><span>三、kafka生产者详解</span></a></h2>
<h3 id="_1、发送类型" tabindex="-1"><a class="header-anchor" href="#_1、发送类型"><span>1、发送类型</span></a></h3>
<ul>
<li>
<p>同步发送</p>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">//异步消息发送</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2、参数详解" tabindex="-1"><a class="header-anchor" href="#_2、参数详解"><span>2、参数详解</span></a></h3>
<ul>
<li><strong>ACK</strong></li>
</ul>
<p>代码的配置方式：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">//ack配置  消息确认机制</span>
...
<p>代码中配置方式：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">//重试次数</span>
...
<p>代码中配置方式：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">//数据压缩</span>
...
<h2 id="四、kafka消费者详解" tabindex="-1"><a class="header-anchor" href="#四、kafka消费者详解"><span>四、kafka消费者详解</span></a></h2>
<h4 id="_1、消费者组" tabindex="-1"><a class="header-anchor" href="#_1、消费者组"><span>1、消费者组</span></a></h4>
<ul>
<li>
...
<h4 id="_2、消息有序性" tabindex="-1"><a class="header-anchor" href="#_2、消息有序性"><span>2、消息有序性</span></a></h4>
<p>应用场景：</p>
<ul>
<li>
...
<figure><img src="/image\spring\spring30.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>topic分区中消息只能由消费者组中的唯一一个消费者处理，所以消息肯定是按照先后顺序进行处理的。但是它也仅仅是保证Topic的一个分区顺序处理，不能保证跨分区的消息先后处理顺序。</p>
<p>Kafka 提供了有限度的顺序性保证，具体来说:</p>
<p>Kafka 通过分区(Pariton)机制和**消息键(MesageKey)**来保证消息的顺序性。在Kafka 中，每个Topic可以分为多个分区，每个分区内的消息都是有序的。</p>
<ul>
<li>
<p>1)在同一个分区内，消息是有序的。</p>
</li>
<li>
<p>2)靠消息键将相关消息分配到同一分区，可以保证这些消息在同一分区内依然有序。</p>
</li>
</ul>
<div class="hint-container tip">
<p class="hint-container-title">消息键(Message Key)和分区策略:</p>
<p>当生产者发送消息时，可以通过配置分区策略(Partitioner)决定消息去哪个分区。默认的分区策略是基于消息键的哈希值，比如 <code v-pre>key.hashcode()%partitionCount </code>。通过这种策略，可以确保相同键的消息被发送到同一个分区，从而保证它们的顺序性。或者采用<strong>只提供一个分区</strong>的方式保整顺序性。</p>
</div>
<h4 id="_3、提交和偏移量" tabindex="-1"><a class="header-anchor" href="#_3、提交和偏移量"><span>3、提交和偏移量</span></a></h4>
<p>kafka不会像其他JMS队列那样需要得到消费者的确认，消费者可以使用kafka来追踪消息在分区的位置（偏移量）消费者会往一个叫做_consumer_offset的特殊主题发送消息，消息里包含了每个分区的偏移量。如果消费者发生崩溃或有新的消费者加入群组，就会触发再均衡</p>
<p><strong>问题一：</strong></p>
<figure><img src="/image\spring\spring31.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>如果提交偏移量小于客户端处理的最后一个消息的偏移量，那么处于两个偏移量之间的消息就会被重复处理。</p>
<p><strong>问题二：</strong></p>
<figure><img src="/image\spring\spring32.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>如果提交的偏移量大于客户端的最后一个消息的偏移量，那么处于两个偏移量之间的消息将会丢失。</p>
<p>如果想要解决这些问题，还要知道目前kafka提交偏移量的方式：提交偏移量的方式有两种，分别是自动提交偏移量和手动提交</p>
<ul>
<li>
<p><strong>自动提交偏移量</strong></p>
<ul>
<li>当enable.auto.commit被设置为true，提交方式就是让消费者自动提交偏移量，每隔5秒消费者会自动把从poll()方法接收的最大偏移量提交上去</li>
</ul>
</li>
<li>
<p><strong>手动提交</strong> ，当enable.auto.commit被设置为false可以有以下三种提交方式</p>
<ul>
<li>
<p>提交当前偏移量（同步提交）</p>
</li>
...
<p><strong>1.提交当前偏移量（同步提交）</strong></p>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">while</span> <span class="token punctuation">(</span><span class="token boolean">true</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2.异步提交</strong></p>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">while</span> <span class="token punctuation">(</span><span class="token boolean">true</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>3.同步和异步组合提交</strong></p>
...
<span class="token punctuation">}</span><span class="token keyword">finally</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="五、springboot集成kafka" tabindex="-1"><a class="header-anchor" href="#五、springboot集成kafka"><span>五、springboot集成kafka</span></a></h2>
<h3 id="_1、入门" tabindex="-1"><a class="header-anchor" href="#_1、入门"><span>1、入门</span></a></h3>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">HelloListener</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2、传递消息为对象" tabindex="-1"><a class="header-anchor" href="#_2、传递消息为对象"><span>2、传递消息为对象</span></a></h3>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@GetMapping</span><span class="token punctuation">(</span><span class="token string">"/hello"</span><span class="token punctuation">)</span>
<span class="token keyword">public</span> <span class="token class-name">String</span> <span class="token function">hello</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">HelloListener</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="六、kafka-stream" tabindex="-1"><a class="header-anchor" href="#六、kafka-stream"><span>六、Kafka Stream</span></a></h2>
...
<p><strong>Kafka Stream的特点如下：</strong></p>
<ul>
...
<figure><img src="/image\spring\spring33.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<h3 id="_2、-kafka-streams的关键概念" tabindex="-1"><a class="header-anchor" href="#_2、-kafka-streams的关键概念"><span>2、 Kafka Streams的关键概念</span></a></h3>
<ul>
...
<figure><img src="/image\spring\spring34.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<h3 id="_3、-kstream" tabindex="-1"><a class="header-anchor" href="#_3、-kstream"><span>3、 KStream</span></a></h3>
<p>（1）数据结构类似于map,如下图，key-value键值对</p>
<figure><img src="/image\spring\spring35.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>（2）KStream</p>
<p><strong>KStream</strong>数据流（data stream），即是一段顺序的，可以无限长，不断更新的数据集。数据流中比较常记录的是事件，这些事件可以是一次鼠标点击（click），一次交易，或是传感器记录的位置数据。</p>
<p>KStream负责抽象的，就是数据流。与Kafka自身topic中的数据一样，类似日志，每一次操作都是<strong>向其中插入（insert）新数据。</strong></p>
<p>为了说明这一点，让我们想象一下以下两个数据记录正在发送到流中：</p>
<p>（“ alice”，1）-&gt;（“” alice“，3）</p>
<p>如果您的流处理应用是要总结每个用户的价值，它将返回<code v-pre>4</code>了<code v-pre>alice</code>。为什么？因为第二条数据记录将不被视为先前记录的更新。（insert）新数据</p>
<h3 id="_4、-kafka-stream入门案例编写" tabindex="-1"><a class="header-anchor" href="#_4、-kafka-stream入门案例编写"><span>4、 Kafka Stream入门案例编写</span></a></h3>
<p>（1）需求分析，求单词个数（word count）</p>
<figure><img src="/image\spring\spring36.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>（2）引入依赖</p>
...
<div class="language-xml line-numbers-mode" data-ext="xml" data-title="xml"><pre v-pre class="language-xml"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependency</span><span class="token punctuation">></span></span>
    ...
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>exclusions</span><span class="token punctuation">></span></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>(3)创建原生的kafka staream入门案例</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">com<span class="token punctuation">.</span>heima<span class="token punctuation">.</span>kafka<span class="token punctuation">.</span>sample</span><span class="token punctuation">;</span>
...
<ul>
<li>
<p>使用生产者在topic为：itcast_topic_input中发送多条消息</p>
</li>
<li>
<p>使用消费者接收topic为：itcast_topic_out</p>
</li>
</ul>
<p>结果：</p>
<ul>
...
<h3 id="_5、-springboot集成kafka-stream" tabindex="-1"><a class="header-anchor" href="#_5、-springboot集成kafka-stream"><span>5、 SpringBoot集成Kafka Stream</span></a></h3>
<p>（1）自定配置参数</p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">KafkaStreamConfig</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>修改application.yml文件，在最下方添加自定义配置</p>
<div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre v-pre class="language-yaml"><code><span class="token key atrule">kafka</span><span class="token punctuation">:</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>(2)新增配置类，创建KStream对象，进行聚合</p>
