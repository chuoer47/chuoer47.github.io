<template><div><!--more--->
<h1 id="mq高级" tabindex="-1"><a class="header-anchor" href="#mq高级"><span>MQ高级</span></a></h1>
<h2 id="一、问题引入" tabindex="-1"><a class="header-anchor" href="#一、问题引入"><span>一、问题引入</span></a></h2>
...
<h2 id="二、发送者的可靠性" tabindex="-1"><a class="header-anchor" href="#二、发送者的可靠性"><span>二、发送者的可靠性</span></a></h2>
<p>消息从发送者发送消息，到消费者处理消息，需要经过的流程是这样的</p>
<figure><img src="/image/cloud/cloud36.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>消息从生产者到消费者的每一步都可能导致消息丢失：</p>
<ul>
<li><strong>发送消息时丢失：</strong>
<ul>
...
<li><strong>MQ导致消息丢失：</strong>
<ul>
<li>消息到达MQ，保存到队列后，尚未消费就突然宕机</li>
</ul>
</li>
<li><strong>消费者处理消息时</strong>：
<ul>
<li>消息接收后尚未处理突然宕机</li>
...
<p class="hint-container-title">综上，我们要解决消息丢失问题，保证MQ的可靠性，就必须从3个方面入手：</p>
<ul>
...
<h3 id="_1、生产者重试机制" tabindex="-1"><a class="header-anchor" href="#_1、生产者重试机制"><span>1、生产者重试机制</span></a></h3>
...
<div class="language-YAML line-numbers-mode" data-ext="YAML" data-title="YAML"><pre v-pre class="language-YAML"><code>spring:
  rabbitmq:
...
<h3 id="_2、生产者确认机制" tabindex="-1"><a class="header-anchor" href="#_2、生产者确认机制"><span>2、生产者确认机制</span></a></h3>
...
<p>针对上述情况，RabbitMQ提供了生产者消息确认机制，包括<code v-pre>Publisher Confirm</code>和<code v-pre>Publisher Return</code>两种。在开启确认机制的情况下，当生产者发送消息给MQ后，MQ会根据消息处理的情况返回不同的<strong>回执</strong>。</p>
<p>具体如图所示：</p>
<figure><img src="/image/cloud/cloud37.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<div class="hint-container info">
<p class="hint-container-title">总结如下：</p>
<ul>
...
<p>其中<code v-pre>ack</code>和<code v-pre>nack</code>属于<strong>Publisher Confirm</strong>机制，<code v-pre>ack</code>是投递成功；<code v-pre>nack</code>是投递失败。而<code v-pre>return</code>则属于<strong>Publisher Return</strong>机制。</p>
<p>默认两种机制都是关闭状态，需要通过配置文件来开启。</p>
<h4 id="_1-开启生产者确认" tabindex="-1"><a class="header-anchor" href="#_1-开启生产者确认"><span>1）开启生产者确认</span></a></h4>
...
<h4 id="_2-定义returncallback" tabindex="-1"><a class="header-anchor" href="#_2-定义returncallback"><span>2）定义ReturnCallback</span></a></h4>
<p>每个<code v-pre>RabbitTemplate</code>只能配置一个<code v-pre>ReturnCallback</code>，因此我们可以在配置类中统一设置。我们在publisher模块定义一个配置类：</p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">MqConfig</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_3-定义confirmcallback" tabindex="-1"><a class="header-anchor" href="#_3-定义confirmcallback"><span>3）定义ConfirmCallback</span></a></h4>
<p>由于每个消息发送时的处理逻辑不一定相同，因此ConfirmCallback需要在每次发消息时定义。具体来说，是在调用RabbitTemplate中的convertAndSend方法时，多传递一个CorrelationData参数，其中包含两个核心的东西：</p>
<ul>
<li><code v-pre>id</code>：消息的唯一标示，MQ对不同的消息的回执以此做判断，避免混淆</li>
<li><code v-pre>SettableListenableFuture</code>：回执结果的Future对象</li>
</ul>
<p>将来MQ的回执就会通过这个<code v-pre>Future</code>来返回，我们可以提前给<code v-pre>CorrelationData</code>中的<code v-pre>Future</code>添加回调函数来处理消息回执：</p>
...
<h2 id="三、mq的可靠性" tabindex="-1"><a class="header-anchor" href="#三、mq的可靠性"><span>三、MQ的可靠性</span></a></h2>
<p>消息到达MQ以后，如果MQ不能及时保存，也会导致消息丢失，所以MQ的可靠性也非常重要。</p>
<h3 id="_1、数据持久化" tabindex="-1"><a class="header-anchor" href="#_1、数据持久化"><span>1、数据持久化</span></a></h3>
...
<h4 id="_1-交换机持久化" tabindex="-1"><a class="header-anchor" href="#_1-交换机持久化"><span>1）交换机持久化</span></a></h4>
<p>在控制台的<code v-pre>Exchanges</code>页面，添加交换机时可以配置交换机的<code v-pre>Durability</code>参数：设置为<code v-pre>Durable</code>就是持久化模式，<code v-pre>Transient</code>就是临时模式。</p>
<h4 id="_2-队列持久化" tabindex="-1"><a class="header-anchor" href="#_2-队列持久化"><span>2）队列持久化</span></a></h4>
<p>在控制台的Queues页面，添加队列时，同样可以配置队列的<code v-pre>Durability</code>参数：</p>
<p>除了持久化以外，队列还有很多其它参数。</p>
<h4 id="_3-消息持久化" tabindex="-1"><a class="header-anchor" href="#_3-消息持久化"><span>3）消息持久化</span></a></h4>
<p>在控制台发送消息的时候，可以添加很多参数，而消息的持久化是要配置一个<code v-pre>properties</code>：</p>
...
<h3 id="_2、lazyqueue" tabindex="-1"><a class="header-anchor" href="#_2、lazyqueue"><span>2、LazyQueue</span></a></h3>
...
<h4 id="_1-控制台配置lazy模式" tabindex="-1"><a class="header-anchor" href="#_1-控制台配置lazy模式"><span>1）控制台配置Lazy模式</span></a></h4>
<p>在添加队列的时候，添加<code v-pre>x-queue-mod=lazy</code>参数即可设置队列为Lazy模式</p>
<h4 id="_2-代码配置lazy模式" tabindex="-1"><a class="header-anchor" href="#_2-代码配置lazy模式"><span>2）代码配置Lazy模式</span></a></h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里是通过<code v-pre>QueueBuilder</code>的<code v-pre>lazy()</code>函数配置Lazy模式</p>
<p>当然，我们也可以基于注解来声明队列并设置为Lazy模式：</p>
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>@RabbitListener(queuesToDeclare = @Queue(
        name = &quot;lazy.queue&quot;,
        durable = &quot;true&quot;,
...
<p>可以基于命令行设置policy：</p>
<div class="language-Shell line-numbers-mode" data-ext="Shell" data-title="Shell"><pre v-pre class="language-Shell"><code>rabbitmqctl set_policy Lazy &quot;^lazy-queue$&quot; '{&quot;queue-mode&quot;:&quot;lazy&quot;}' --apply-to queues  
...
<p>当然，也可以在控制台配置policy，进入在控制台的<code v-pre>Admin</code>页面，点击<code v-pre>Policies</code>，即可添加配置</p>
<h2 id="四、消费者的可靠性" tabindex="-1"><a class="header-anchor" href="#四、消费者的可靠性"><span>四、消费者的可靠性</span></a></h2>
...
<h3 id="_1、消费者确认机制" tabindex="-1"><a class="header-anchor" href="#_1、消费者确认机制"><span>1、消费者确认机制</span></a></h3>
<p>为了确认消费者是否成功处理消息，RabbitMQ提供了消费者确认机制（<strong>Consumer Acknowledgement</strong>）。即：当消费者处理消息结束后，应该向RabbitMQ发送一个回执，告知RabbitMQ自己消息处理状态。</p>
<div class="hint-container info">
<p class="hint-container-title">回执有三种可选值：</p>
<ul>
...
<ul>
<li><strong><code v-pre>none</code></strong>：不处理。即消息投递给消费者后立刻ack，消息会立刻从MQ删除。非常不安全，不建议使用</li>
<li><strong><code v-pre>manual</code></strong>：手动模式。需要自己在业务代码中调用api，发送<code v-pre>ack</code>或<code v-pre>reject</code>，存在业务入侵，但更灵活</li>
<li><strong><code v-pre>auto</code></strong>：自动模式。SpringAMQP利用AOP对我们的消息处理逻辑做了环绕增强，当业务正常执行时则自动返回<code v-pre>ack</code>.  当业务出现异常时，根据异常判断返回不同结果：
<ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2、失败重试机制" tabindex="-1"><a class="header-anchor" href="#_2、失败重试机制"><span>2、失败重试机制</span></a></h3>
...
<p>为了应对上述情况Spring又提供了消费者失败重试机制：在消费者出现异常时利用本地重试，而不是无限制的requeue到mq队列。</p>
...
<h3 id="_3、失败处理策略" tabindex="-1"><a class="header-anchor" href="#_3、失败处理策略"><span>3、失败处理策略</span></a></h3>
<p>在之前的测试中，本地测试达到最大重试次数后，消息会被丢弃。这在某些对于消息可靠性要求较高的业务场景下，显然不太合适了。</p>
<p>因此Spring允许我们自定义重试次数耗尽后的消息处理策略，这个策略是由<code v-pre>MessageRecovery</code>接口来定义的，它有3个不同实现：</p>
<ul>
...
<p><strong>1）在consumer服务中定义处理失败消息的交换机和队列</strong></p>
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>@Bean
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2）定义一个RepublishMessageRecoverer，关联队列和交换机</strong></p>
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>@Bean
...
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>@Configuration
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_4、业务幂等性" tabindex="-1"><a class="header-anchor" href="#_4、业务幂等性"><span>4、业务幂等性</span></a></h3>
<p>何为幂等性？</p>
<p><strong>幂等</strong>是一个数学概念，用函数表达式来描述是这样的：<code v-pre>f(x) = f(f(x))</code>，例如求绝对值函数。</p>
...
<p>所以，我们要尽可能避免业务被重复执行。然而在实际业务场景中，由于意外经常会出现业务被重复执行的情况，例如：</p>
<ul>
<li>页面卡顿时频繁刷新导致表单重复提交</li>
<li>服务间调用的重试</li>
<li>MQ消息的重复投递</li>
</ul>
<p>我们必须想办法保证消息处理的幂等性。</p>
<div class="hint-container tip">
<p class="hint-container-title">这里给出两种方案：</p>
<ul>
<li>唯一消息ID</li>
<li>业务状态判断</li>
</ul>
</div>
<h4 id="_1-唯一消息id" tabindex="-1"><a class="header-anchor" href="#_1-唯一消息id"><span>1）唯一消息ID</span></a></h4>
<p>这个思路非常简单：</p>
<ol>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_2-业务判断" tabindex="-1"><a class="header-anchor" href="#_2-业务判断"><span>2）业务判断</span></a></h4>
...
<div class="language-SQL line-numbers-mode" data-ext="SQL" data-title="SQL"><pre v-pre class="language-SQL"><code>UPDATE `order` SET status = ? , pay_time = ? WHERE id = ? AND status = 1
...
<h3 id="_5、兜底方案" tabindex="-1"><a class="header-anchor" href="#_5、兜底方案"><span>5、兜底方案</span></a></h3>
...
<p>流程如下：</p>
<figure><img src="/image/cloud/cloud38.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
<div class="hint-container info">
<p class="hint-container-title">综上，支付服务与交易服务之间的订单状态一致性是如何保证的？</p>
<ul>
...
<h2 id="五、延迟消息" tabindex="-1"><a class="header-anchor" href="#五、延迟消息"><span>五、延迟消息</span></a></h2>
<p>在电商的支付业务中，对于一些库存有限的商品，为了更好的用户体验，通常都会在用户下单时立刻扣减商品库存。例如电影院购票、高铁购票，下单后就会锁定座位资源，其他人无法重复购买。但是这样就存在一个问题，假如用户下单后一直不付款，就会一直占有库存资源，导致其他客户无法正常交易，最终导致商户利益受损！因此，电商中通常的做法就是：<strong>对于超过一定时间未支付的订单，应该立刻取消订单并释放占用的库存</strong>。</p>
<p>例如，订单支付超时时间为30分钟，则我们应该在用户下单后的第30分钟检查订单支付状态，如果发现未支付，应该立刻取消订单，释放库存。</p>
<p>但问题来了：如何才能准确的实现在下单后第30分钟去检查支付状态呢？像这种在一段时间以后才执行的任务，我们称之为<strong>延迟任务</strong>，而要实现延迟任务，最简单的方案就是利用MQ的延迟消息了。</p>
<div class="hint-container tip">
<p class="hint-container-title">在RabbitMQ中实现延迟消息也有两种方案：</p>
<ul>
<li>死信交换机+TTL</li>
<li>延迟消息插件</li>
</ul>
</div>
<h3 id="_1、死信交换机" tabindex="-1"><a class="header-anchor" href="#_1、死信交换机"><span>1、死信交换机</span></a></h3>
<div class="hint-container tip">
<p class="hint-container-title">什么是死信？</p>
...
<p>如果一个队列中的消息已经成为死信，并且这个队列通过**<code v-pre>dead-letter-exchange</code><strong>属性指定了一个交换机，那么队列中的死信就会投递到这个交换机中，而这个交换机就称为</strong>死信交换机**（Dead Letter Exchange）。而此时加入有队列与死信交换机绑定，则最终死信就会被投递到这个队列中。</p>
<div class="hint-container info">
<p class="hint-container-title">死信交换机有什么作用呢？</p>
<ol>
...
<ul>
<li>前面两种作用场景可以看做是把死信交换机当做一种消息处理的最终兜底方案，与消费者重试时讲的<code v-pre>RepublishMessageRecoverer</code>作用类似。</li>
</ul>
<p>而最后一种场景：如图，有一组绑定的交换机（<code v-pre>ttl.fanout</code>）和队列（<code v-pre>ttl.queue</code>）。但是<code v-pre>ttl.queue</code>没有消费者监听，而是设定了死信交换机<code v-pre>hmall.direct</code>，而队列<code v-pre>direct.queue1</code>则与死信交换机绑定，RoutingKey是blue：</p>
<figure><img src="/image/cloud/cloud39.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>假如我们现在发送一条消息到<code v-pre>ttl.fanout</code>，RoutingKey为blue，并设置消息的<strong>有效期</strong>为5000毫秒：</p>
<figure><img src="/image/cloud/cloud40.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
<p>消息肯定会被投递到<code v-pre>ttl.queue</code>之后，由于没有消费者，因此消息无人消费。5秒之后，消息的有效期到期，成为死信</p>
<p>死信被再次投递到死信交换机<code v-pre>hmall.direct</code>，并沿用之前的RoutingKey，也就是<code v-pre>blue</code>由于<code v-pre>direct.queue1</code>与<code v-pre>hmall.direct</code>绑定的key是blue，因此最终消息被成功路由到<code v-pre>direct.queue1</code>，如果此时有消费者与<code v-pre>direct.queue1</code>绑定， 也就能成功消费消息了。但此时已经是5秒钟以后了</p>
...
<p>1）基于注解方式生成<code v-pre>ttl.fanout</code>和<code v-pre>ttl.queue</code>并进行绑定，<code v-pre>ttl.queue</code>绑定死信交换机<code v-pre>dls.direct</code></p>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    <span class="token annotation punctuation">@RabbitListener</span><span class="token punctuation">(</span>bindings <span class="token operator">=</span> <span class="token annotation punctuation">@QueueBinding</span><span class="token punctuation">(</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>3）向<code v-pre>ttl.fanout</code>发送一条消息到，RoutingKey为blue，并设置消息的<strong>有效期</strong>为5000毫秒</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    <span class="token annotation punctuation">@Test</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2、delayexchange插件" tabindex="-1"><a class="header-anchor" href="#_2、delayexchange插件"><span>2、DelayExchange插件</span></a></h3>
<p>基于死信队列虽然可以实现延迟消息，但是太麻烦了。因此RabbitMQ社区提供了一个延迟消息插件来实现相同的效果。<a href="https://www.rabbitmq.com/blog/2015/04/16/scheduling-messages-with-rabbitmq" target="_blank" rel="noopener noreferrer">官方文档</a></p>
<h4 id="_1-下载" tabindex="-1"><a class="header-anchor" href="#_1-下载"><span>1）下载</span></a></h4>
<p><a href="https://github.com/rabbitmq/rabbitmq-delayed-message-exchange" target="_blank" rel="noopener noreferrer">插件下载地址：</a></p>
<p>由于我们安装的MQ是<code v-pre>3.8</code>版本，因此这里下载<code v-pre>3.8.17</code>版本：</p>
<p>因为我们是基于Docker安装，所以需要先查看RabbitMQ的插件目录对应的数据卷。</p>
<div class="language-Shell line-numbers-mode" data-ext="Shell" data-title="Shell"><pre v-pre class="language-Shell"><code>docker volume inspect mq-plugins
...
<p>接下来执行命令，安装插件：</p>
<div class="language-Shell line-numbers-mode" data-ext="Shell" data-title="Shell"><pre v-pre class="language-Shell"><code>docker exec -it mq rabbitmq-plugins enable rabbitmq_delayed_message_exchange
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h4 id="_2-声明延迟交换机" tabindex="-1"><a class="header-anchor" href="#_2-声明延迟交换机"><span>2）声明延迟交换机</span></a></h4>
<p><strong>基于注解方式：</strong></p>
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>@RabbitListener(bindings = @QueueBinding(
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>基于<code v-pre>@Bean</code>的方式：</strong></p>
<div class="language-Java line-numbers-mode" data-ext="Java" data-title="Java"><pre v-pre class="language-Java"><code>package com.itheima.consumer.config;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3、发送延迟消息" tabindex="-1"><a class="header-anchor" href="#_3、发送延迟消息"><span>3、发送延迟消息</span></a></h3>
