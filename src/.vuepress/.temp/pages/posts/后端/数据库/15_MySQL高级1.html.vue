<template><div><!--more--->
<h1 id="mysql高级一" tabindex="-1"><a class="header-anchor" href="#mysql高级一"><span>MySQL高级一</span></a></h1>
<h2 id="一、基础" tabindex="-1"><a class="header-anchor" href="#一、基础"><span>一、基础</span></a></h2>
<h3 id="_1、mysql体系结构" tabindex="-1"><a class="header-anchor" href="#_1、mysql体系结构"><span>1、MySQL体系结构</span></a></h3>
...
<ul>
<li>用户名密码验证通过后，连接器会到权限表中查出所拥有的权限（因此修改权限不会立即生效） <code v-pre>mysql -uroot -p</code></li>
</ul>
</li>
<li>查询缓存(仅适用于不常修改的数据，且 MySQL 8 中已经删除缓存模块)</li>
<li>分析器：词法分析、语法分析，例如识别表名、列名等</li>
<li>优化器：存在多个索引时决定使用哪个索引，或多表关联时决定表的连接顺序，以确定语句的执行计划</li>
<li>执行器：校验权限后，真正执行语句，将执行结果存入结果集，最后返回</li>
</ul>
<h3 id="_2、存储引擎" tabindex="-1"><a class="header-anchor" href="#_2、存储引擎"><span>2、存储引擎</span></a></h3>
<p>存储引擎就是存储数据、建立索引、更新/查询数据等技术的实现方式。存储引擎基于表而非基于库，因此也被称为表引擎。</p>
<p>相关语法：</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 查询建表语句</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><center><img src="/image/mysql/mysql5.png" alt="mysql存储引擎" style="zoom: 33%;" /></center>
<h4 id="_1-innodb" tabindex="-1"><a class="header-anchor" href="#_1-innodb"><span>1)InnoDB</span></a></h4>
...
<li>
<p>innodb_file_per_table 参数决定多张表共用一个系统表空间还是每张表独立表空间 <code v-pre>show variables like 'innodb_file_per_table';</code></p>
<ul>
<li>从idb文件提取表结构数据的工具： <code v-pre>ibd2sdi xxx.ibd</code></li>
</ul>
</li>
</ul>
<h4 id="_2-myisam" tabindex="-1"><a class="header-anchor" href="#_2-myisam"><span>2)MyISAM</span></a></h4>
...
<p>xxx.MYD: 存储数据</p>
<p>xxx.MYI: 存储索引</p>
<h4 id="_3-memory" tabindex="-1"><a class="header-anchor" href="#_3-memory"><span>3)Memory</span></a></h4>
...
<p><strong>文件:</strong></p>
<p>xxx.sdi：存储表结构信息</p>
<h3 id="_3、存储引擎区别和特点" tabindex="-1"><a class="header-anchor" href="#_3、存储引擎区别和特点"><span>3、存储引擎区别和特点</span></a></h3>
<table>
<thead>
<tr>
...
<h3 id="_4、存储引擎选择" tabindex="-1"><a class="header-anchor" href="#_4、存储引擎选择"><span>4、存储引擎选择</span></a></h3>
<ul>
...
<h3 id="_1、索引概述" tabindex="-1"><a class="header-anchor" href="#_1、索引概述"><span>1、索引概述</span></a></h3>
<p>如果没有索引，只能依次遍历所有记录，效率十分低下。除了实际存储的数据，DBS 还维护着满足特定查找算法的数据结构，这些数据结构以某种方式引用（指向）数据，可以借此实现高效的查找算法。这种数据结构就是索引</p>
<p>优点：</p>
<ul>
...
<ul>
<li>索引列需要占用额外空间</li>
<li>索引大大提高了查询效率，但降低了增删改的速度</li>
</ul>
<p>MySQL的索引是在<strong>存储引擎层</strong>实现的，不同的存储引擎支持不同的索引结构，主要包含以下几种：</p>
<table>
<thead>
<tr>
<th>索引结构</th>
...
<td style="text-align:center">5.6版本后支持</td>
<td style="text-align:center">支持</td>
<td style="text-align:center">不支持</td>
</tr>
</tbody>
</table>
<p>注：如果没有特别指明，一般指<code v-pre>B+树</code>结构组织的索引。</p>
<h3 id="_2、索引结构" tabindex="-1"><a class="header-anchor" href="#_2、索引结构"><span>2、索引结构</span></a></h3>
<h4 id="_1-b-树" tabindex="-1"><a class="header-anchor" href="#_1-b-树"><span>1)B+树</span></a></h4>
<ul>
<li>B+Tree是B-Tree的变种</li>
<li>非叶子节点仅作为索引，存储所指向的数据页中的最小键，不存储数据</li>
<li>叶子节点存储实际数据，且所有叶子节点形成一个单向链表</li>
</ul>
<center><img src="/image/mysql/mysql6.png"  style="zoom: 33%;" /></center>
<ul>
<li>MySQL索引数据结构对经典的B+Tree进行了优化。每层节点按照索引值从小到大的顺序排序而组成了双向链表，每个页内的记录按索引列排序形成单链表。</li>
<li>所有结点本质上都是一个数据页，一页至少2条记录。目录项记录只存储主键值和对应的页号。</li>
<li>非叶子结点中的记录头<code v-pre>record_type</code>存1标识记录项记录，叶子结点中的记录头<code v-pre>record_type</code>存0标识数据记录</li>
<li>实际新增数据时，从根节点开始，存满后页分裂，向下生长树枝。因此根节点始终保持不动，并存储在数据字典中。</li>
<li>这种B+Tree提高了区间访问的性能，利于排序</li>
<li>另外，MyISAM中索引和数据分离，叶子节点存储的都是数据记录的地址，因此MyISAM的索引都是二级索引</li>
</ul>
<center><img src="/image/mysql/mysql7.png"  style="zoom: 50%;" /></center>
<h4 id="_2-hash索引" tabindex="-1"><a class="header-anchor" href="#_2-hash索引"><span>2)Hash索引</span></a></h4>
<ul>
...
<li>特点：
<ul>
<li>Hash索引只能用于对等比较<code v-pre>(=，in)</code>，不支持范围查询<code v-pre>(between，&gt;，&lt; ，...)</code></li>
<li>无法利用索引完成排序操作</li>
...
<h3 id="_3、索引分类" tabindex="-1"><a class="header-anchor" href="#_3、索引分类"><span>3、索引分类</span></a></h3>
<ul>
<li>
<p>MySQL中，索引的具体类型主要分为以下几类：</p>
<table>
<thead>
<tr>
<th>分类</th>
...
<td>将数据存储与索引放一块，索引结构的叶子节点直接保存了行数据</td>
<td>必须有且只有一个；索引即数据，数据即索引。</td>
</tr>
<tr>
<td>二级索引 (Secondary Index)</td>
<td>非主键生成的索引，索引结构的叶子节点关联的是对应的主键</td>
<td>可以存在多个；需要回表查询</td>
</tr>
</tbody>
</table>
...
<li>如果表没有主键或没有合适的唯一索引，则使用 InnoDB 自动生成的<code v-pre>rowid</code>作为隐藏的聚集索引</li>
</ul>
</li>
</ul>
...
<li>二级索引的叶子节点下挂的是该字段值对应的主键值（实际非叶子节点为了保证目录项唯一，也要保存主键值）</li>
<li><strong>回表查询过程： 二级索引 -&gt; 主键值 -&gt; 聚集索引</strong></li>
</ul>
...
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<p>在业务场景中，如果存在多个查询条件，考虑针对查询字段建立索引时，建议建立联合索引，而非单列索引。多条件联合查询时，MySQL优化器会评估哪个字段的索引效率更高，选择该索引完成本次查询。</p>
</div>
<p>联合索引结构图：</p>
<center><img src="/image/mysql/mysql11.png"  style="zoom:50%;" /></center>
<h3 id="_4、索引语法" tabindex="-1"><a class="header-anchor" href="#_4、索引语法"><span>4、索引语法</span></a></h3>
<ul>
<li>创建索引：
<code v-pre>CREATE [ UNIQUE | FULLTEXT ] INDEX index_name ON table_name (index_col_name, ...);</code>
如果 CREATE 后面不加索引类型参数，则创建的是常规索引</li>
<li>查看索引：
<code v-pre>SHOW INDEX FROM table_name;</code></li>
<li>删除索引：
<code v-pre>DROP INDEX index_name ON table_name;</code></li>
</ul>
<p>示例：</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- name字段为姓名字段，该字段的值可能会重复，为该字段创建索引</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_5、索引使用" tabindex="-1"><a class="header-anchor" href="#_5、索引使用"><span>5、索引使用</span></a></h3>
<h4 id="_1-最左前缀法则" tabindex="-1"><a class="header-anchor" href="#_1-最左前缀法则"><span>1)最左前缀法则</span></a></h4>
<ul>
...
<h4 id="_2-索引失效情况" tabindex="-1"><a class="header-anchor" href="#_2-索引失效情况"><span>2)索引失效情况</span></a></h4>
<ol>
<li>在索引列上进行运算操作，索引将失效 如：<code v-pre>explain select * from tb_user where substring(phone, 10, 2) = '15';</code></li>
<li>字符串类型不加引号，索引将失效 如：<code v-pre>explain select * from tb_user where phone = 17799990015;</code></li>
<li>模糊查询中对头部模糊匹配，索引将失效 如：<code v-pre>explain select * from tb_user where profession like '%工程';</code>， 对前后都模糊匹配也会失效：<code v-pre>explain select * from tb_user where profession like '%工%';</code> 仅对尾部模糊匹配不会失效：<code v-pre>explain select * from tb_user where profession like '软件%';</code></li>
<li>用 or 连接的条件，左右两侧字段都有索引时，索引才会生效 因为只要有一个没有索引，另一个用不用索引没有意义，仍要进行全表扫描，所以无需用索引。</li>
<li>数据分布的影响 如果 MySQL 评估使用索引比全表更慢，则不使用索引。因为索引是用来索引少量数据的，如果通过索引查询返回大批量的数据，则还不如走全表扫描来的快，此时索引就会失效。</li>
</ol>
<h4 id="_3-优化方案" tabindex="-1"><a class="header-anchor" href="#_3-优化方案"><span>3)优化方案</span></a></h4>
<p><strong>指定索引</strong></p>
<p>优化数据库的一个重要手段，在SQL语句中加入一些手动提示，优化MySQL的索引使用策略，以提升性能</p>
<ul>
<li>建议使用索引 - use index <code v-pre>explain select * from tb_user use index(idx_user_pro) where profession=&quot;软件工程&quot;;</code></li>
<li>忽略索引 - ignore index <code v-pre>explain select * from tb_user ignore index(idx_user_pro) where profession=&quot;软件工程&quot;;</code></li>
<li>强制使用索引 - force index <code v-pre>explain select * from tb_user force index(idx_user_pro) where profession=&quot;软件工程&quot;;</code></li>
</ul>
<p><strong>覆盖索引</strong></p>
<p>应尽量使用覆盖索引，减少 <code v-pre>select *</code>。即需要查询的数据在单个索引结构中能够全部获取到，避免回表查询</p>
<p>explain 执行计划中 extra 字段含义：</p>
<ul>
<li><code v-pre>using index condition</code>：查找使用了索引，但是需要回表查询数据</li>
<li><code v-pre>using where; using index;</code>：查找使用了索引，但是需要的数据都在索引列中能找到，不需要回表查询。因此性能更高</li>
</ul>
<h3 id="_6、索引设计原则" tabindex="-1"><a class="header-anchor" href="#_6、索引设计原则"><span>6、索引设计原则</span></a></h3>
<ol>
<li>针对数据量较大，且查询比较频繁的表建立索引</li>
<li>针对常作为查询条件（where）、排序（order by）、分组（group by）操作的字段建立索引</li>
<li>考虑列的基数(区分度)，最好为基数大的列建立索引</li>
<li>索引列的类型尽量小</li>
<li>如果是字符串类型的字段，字段长度较长，可以针对字段的特点，建立前缀索引</li>
<li>尽量使用联合索引，减少单列索引。查询时，联合索引很多时候可以覆盖索引，节省存储空间，避免回表，提高查询效率</li>
<li>避免冗余和重复索引，索引并不是多多益善，索引越多，维护索引结构的代价就越大，会影响增删改的效率</li>
<li>如果索引列不能存储NULL值，请在创建表时使用NOT NULL约束它。当优化器知道每列是否包含NULL值时，它可以更好地确定哪个索引最有效地用于查询</li>
<li>主键按序插入能够提高性能，最好定义auto_increment</li>
</ol>
<h2 id="三、sql优化" tabindex="-1"><a class="header-anchor" href="#三、sql优化"><span>三、SQL优化</span></a></h2>
<h3 id="_1、sql性能分析" tabindex="-1"><a class="header-anchor" href="#_1、sql性能分析"><span>1、SQL性能分析</span></a></h3>
<h4 id="_1-sql执行频率" tabindex="-1"><a class="header-anchor" href="#_1-sql执行频率"><span>1)<strong>SQL</strong>执行频率</span></a></h4>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Com_delete: 删除次数、Com_insert: 插入次数、Com_select: 查询次数、Com_update: 更新次数</p>
<h4 id="_2-慢查询日志" tabindex="-1"><a class="header-anchor" href="#_2-慢查询日志"><span>2)慢查询日志</span></a></h4>
<p>慢查询日志记录了所有执行时间超过指定参数（long_query_time，单位：秒，默认10秒）的所有SQL语句的日志。MySQL的慢查询日志默认没有开启，我们可以查看一下系统变量 slow_query_log。</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">show</span> variables <span class="token operator">like</span> <span class="token string">'slow_query_log'</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如果要开启慢查询日志，需要在MySQL的配置文件（/etc/my.cnf）中配置如下信息：</p>
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 开启MySQL慢日志查询开关</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>配置完毕之后，重新启动MySQL服务器进行测试，查看慢日志文件中记录的信息<code v-pre>/var/lib/mysql/localhost-slow.log</code>。</p>
<h4 id="_3-profile详情" tabindex="-1"><a class="header-anchor" href="#_3-profile详情"><span>3)<strong>profile</strong>详情</span></a></h4>
<p><code v-pre>show profiles</code> 能够在做SQL优化时帮助我们了解时间都耗费到哪里去了。通过<code v-pre>have_profiling</code>参数，能够看到当前MySQL是否支持profile操作：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>YES表示MySQL是支持 profile操作的，但是开关是关闭的。可以通过set语句在session/global级别开启profiling：</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">SET</span> profiling <span class="token operator">=</span> <span class="token number">1</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>接下来，我们所执行的SQL语句，都会被MySQL记录，并记录执行时间消耗到哪儿去了</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_4-explain" tabindex="-1"><a class="header-anchor" href="#_4-explain"><span>4)<strong>explain</strong></span></a></h4>
<p><code v-pre>EXPLAIN</code> 或者 <code v-pre>DESC</code>命令获取 MySQL 如何执行 SELECT 语句的信息，包括在 SELECT 语句执行过程中表如何连接和连接的顺序。</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 直接在select语句之前加上关键字 explain / desc</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>Explain 执行计划中各个字段的含义:</p>
<table>
<thead>
<tr>
<th>字段</th>
...
<tr>
<td>id</td>
<td style="text-align:center">select查询的序列号，表示查询中执行select子句或者是操作表的顺序(id相同，执行顺序从上到下；id不同，值越大，越先执行)。</td>
</tr>
<tr>
<td>select_type</td>
<td style="text-align:center">表示 SELECT 的类型，常见的取值有 SIMPLE（简单表，即不使用表连接或者子查询）、PRIMARY（主查询，即外层的查询）、UNION（UNION 中的第二个或者后面的查询语句）、SUBQUERY（SELECT/WHERE之后包含了子查询）等</td>
</tr>
<tr>
<td>type</td>
<td style="text-align:center">表示连接类型，性能由好到差的连接类型为NULL、system、const、eq_ref、ref、range、 index、all 。</td>
</tr>
<tr>
<td>possible_key</td>
<td style="text-align:center">显示可能应用在这张表上的索引，一个或多个。</td>
</tr>
<tr>
<td>key</td>
...
<tr>
<td>key_len</td>
<td style="text-align:center">表示索引中使用的字节数， 该值为索引字段最大可能长度，并非实际使用长度，在不损失精确性的前提下， 长度越短越好 。</td>
</tr>
<tr>
<td>rows</td>
<td style="text-align:center">MySQL认为必须要执行查询的行数，在innodb引擎的表中，是一个估计值，可能并不总是准确的。</td>
</tr>
<tr>
<td>filtered</td>
<td style="text-align:center">表示返回结果的行数占需读取行数的百分比， filtered 的值越大越好。</td>
</tr>
</tbody>
</table>
<h3 id="_2、插入数据" tabindex="-1"><a class="header-anchor" href="#_2、插入数据"><span>2、插入数据</span></a></h3>
<h4 id="_1-insert" tabindex="-1"><a class="header-anchor" href="#_1-insert"><span>1)insert</span></a></h4>
...
<li>3、优化方案三主键顺序插入，性能要高于乱序插入。</li>
</ul>
<h4 id="_2-大批量插入数据" tabindex="-1"><a class="header-anchor" href="#_2-大批量插入数据"><span>2)大批量插入数据</span></a></h4>
<p>如果一次性需要插入大批量数据(比如: 几百万的记录)，使用insert语句插入性能较低，此时可以使用MySQL数据库提供的load指令进行插入。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3、主键优化" tabindex="-1"><a class="header-anchor" href="#_3、主键优化"><span>3、主键优化</span></a></h3>
...
<ul>
<li>如果一个页快满了,此时我们插入数据,但下一个页的空间也全部占满.这个时候Mysql将创建一个新页,然后将快满的这个页的部分数据迁移到新页中,这部分数据就是超出原来那个页阈值的那部分数据,之后再插入新的数据,这种现象就是页分裂.</li>
</ul>
<p><strong>页合并:</strong></p>
<ul>
<li>
<p>如果页中的数据被删除,那么实际上这块的空间并不会被回收,而是标记为可重复利用.当一个页的数据被删除或者更新,空间小于所规定的阈值大小,那么Mysql会查找前一个页,和后一个页,判断是否可以将这个页合并到另外一个页,这样就可以节省下一个页的空间.</p>
</li>
<li>
...
<div class="hint-container tip">
<p class="hint-container-title">主键设计原则</p>
<ul>
<li>
<p>满足业务需求的情况下，尽量降低主键的长度。</p>
</li>
<li>
<p>插入数据时，尽量选择顺序插入，选择使用AUTO_INCREMENT自增主键。尽量不要使用UUID做主键或者是其他自然主键，如身份证号。</p>
</li>
<li>
<p>业务操作时，避免对主键的修改。</p>
</li>
...
<h3 id="_4、order-by优化" tabindex="-1"><a class="header-anchor" href="#_4、order-by优化"><span>4、<strong>order by</strong>优化</span></a></h3>
<p>MySQL的排序，有两种方式：</p>
<ul>
<li>
...
<div class="hint-container tip">
<p class="hint-container-title">order by优化原则:</p>
<ul>
<li>
...
<li><code v-pre>create index idx user_age_phone_ad on tb_user(age asc ,phone desc);</code></li>
</ul>
</li>
<li>
...
<h3 id="_5、group-by优化" tabindex="-1"><a class="header-anchor" href="#_5、group-by优化"><span>5、<strong>group by</strong>优化</span></a></h3>
<p>分组操作中，我们需要通过以下两点进行优化，以提升性能：</p>
<ul>
<li>
...
<h3 id="_6、limit优化" tabindex="-1"><a class="header-anchor" href="#_6、limit优化"><span>6、<strong>limit</strong>优化</span></a></h3>
<p>在数据量比较大时，如果进行limit分页查询，在查询时，越往后，分页查询效率越低。</p>
<p><strong>优化思路</strong>: 一般分页查询时，通过创建 覆盖索引 能够比较好地提高性能，可以通过覆盖索引加子查询形式进行优化。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_7、count优化" tabindex="-1"><a class="header-anchor" href="#_7、count优化"><span>7、<strong>count</strong>优化</span></a></h3>
<p>如果数据量很大，在执行count操作时，是非常耗时的。</p>
<ul>
<li>
...
<li>count（*）:
<ul>
<li>InnoDB引擎并不会把全部字段取出来，而是专门做了优化，<strong>不取值，服务层直接按行进行累加</strong>。</li>
</ul>
</li>
<li>count（主键）
<ul>
...
<li>count（字段）
<ul>
<li>没有not null 约束 : InnoDB 引擎会遍历整张表把每一行的字段值都取出来，返回给服务层，服务层判断是否为null，不为null，计数累加。有not null 约束：InnoDB 引擎会遍历整张表把每一行的字段值都取出来，返回给服务层，直接按行进行累加。</li>
</ul>
</li>
<li>count（数字）
<ul>
...
<h3 id="_8、update优化" tabindex="-1"><a class="header-anchor" href="#_8、update优化"><span>8、<strong>update</strong>优化</span></a></h3>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">update</span> course <span class="token keyword">set</span> name <span class="token operator">=</span> <span class="token string">'javaEE'</span> <span class="token keyword">where</span> id <span class="token operator">=</span> <span class="token number">1</span> <span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>当我们在执行删除的SQL语句时，会锁定id为1这一行的数据，然后事务提交之后，行锁释放。</p>
<p>但是当我们在执行如下SQL时。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>当我们开启多个事务，在执行上述的SQL时，我们发现行锁升级为了表锁。 导致该update语句的性能大大降低。</p>
<div class="hint-container warning">
<p class="hint-container-title">注意</p>
<p>InnoDB的行锁是针对索引加的锁，不是针对记录加的锁 ,并且该索引不能失效，否则会从行锁升级为表锁 。</p>
</div>
<h2 id="四、视图-存储过程-触发器" tabindex="-1"><a class="header-anchor" href="#四、视图-存储过程-触发器"><span>四、视图/存储过程/触发器</span></a></h2>
<h3 id="_1、视图" tabindex="-1"><a class="header-anchor" href="#_1、视图"><span>1、视图</span></a></h3>
...
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 1、创建</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>检查选项:</strong></p>
...
<ul>
<li>
<p>CASCADED 级联。比如，v2视图是基于v1视图的，如果在v2视图创建的时候指定了检查选项为 cascaded，但是v1视图创建时未指定检查选项。 则在执行检查时，不仅会检查v2，还会级联检查v2的关联视图v1。</p>
</li>
<li>
<p>LOCAL 本地。比如，v2视图是基于v1视图的，如果在v2视图创建的时候指定了检查选项为 local ，但是v1视图创建时未指定检查选项。 则在执行检查时，知会检查v2，不会检查v2的关联视图v1。</p>
</li>
</ul>
...
<div class="hint-container tip">
<p class="hint-container-title">视图的作用</p>
<ul>
<li>
<p>1、<strong>简单</strong>、视图不仅可以简化用户对数据的理解，也可以简化他们的操作。那些被经常使用的查询可以被定义为视图，从而使得用户不必为以后的操作每次指定全部的条件。</p>
</li>
<li>
<p>2、<strong>安全</strong>、数据库可以授权，但不能授权到数据库特定行和特定的列上。通过视图用户只能查询和修改他们所能见到的数据</p>
</li>
<li>
<p>3、<strong>数据独立</strong>、视图可帮助用户屏蔽真实表结构变化带来的影响。</p>
</li>
</ul>
</div>
<h3 id="_2、存储过程" tabindex="-1"><a class="header-anchor" href="#_2、存储过程"><span>2、存储过程</span></a></h3>
<p>存储过程是事先经过编译并存储在数据库中的一段 SQL 语句的集合，调用存储过程可以简化应用开发人员的很多工作，减少数据在数据库和应用服务器之间的传输，对于提高数据处理的效率是有好处的。</p>
<p><strong>存储过程思想上很简单，就是数据库 SQL 语言层面的代码封装与重用。</strong></p>
<h4 id="_1-基本语法" tabindex="-1"><a class="header-anchor" href="#_1-基本语法"><span>1)基本语法</span></a></h4>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code> <span class="token comment">-- 1、创建</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在命令行中，执行创建存储过程的SQL时，需要通过关键字 delimiter 指定SQL语句的结束符。</p>
<h4 id="_2-变量" tabindex="-1"><a class="header-anchor" href="#_2-变量"><span>2)变量</span></a></h4>
<p>在MySQL中变量分为三种类型:</p>
<ul>
<li>系统变量: 是MySQL服务器提供，不是用户定义的，属于服务器层面。分为全局变量（GLOBAL）、会话变量（SESSION）。</li>
</ul>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 1、查看系统变量</span>
...
<li>
<p>全局变量(GLOBAL): 全局变量针对于所有的会话。</p>
</li>
<li>
<p>会话变量(SESSION): 会话变量针对于单个会话，在另外一个会话窗口就不生效了。</p>
</li>
</ul>
</div>
<ul>
<li>用户定义变量: 是用户根据需要自己定义的变量，用户变量不用提前声明，在用的时候直接用 &quot;@变量名&quot; 使用就可以。其作用域为当前连接。</li>
</ul>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 1、赋值,赋值时，可以使用 = ，也可以使用 := 。</span>
<span class="token comment">-- 方式一</span>
...
<ul>
<li>局部变量: 是根据需要定义的在局部生效的变量，访问之前，需要DECLARE声明。可用作存储过程内的局部变量和输入参数，局部变量的范围是在其内声明的BEGIN ... END块。</li>
</ul>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 1、声明</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_3-if语句" tabindex="-1"><a class="header-anchor" href="#_3-if语句"><span>3)if语句</span></a></h4>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">IF</span> 条件<span class="token number">1</span> <span class="token keyword">THEN</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_4-参数" tabindex="-1"><a class="header-anchor" href="#_4-参数"><span>4)参数</span></a></h4>
<p>参数的类型，主要分为以下三种：IN、OUT、INOUT。 具体的含义如下：</p>
<ul>
<li>IN:该类参数作为输入，也就是需要调用时传入值(默认)</li>
<li>OUT:该类参数作为输出，也就是该参数可以作为返回值</li>
<li>INUOUT:既可以作为输入参数，也可以作为输出参数</li>
</ul>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">CREATE</span> <span class="token keyword">PROCEDURE</span> 存储过程名称 <span class="token punctuation">(</span><span class="token punctuation">[</span> <span class="token operator">IN</span><span class="token operator">/</span><span class="token keyword">OUT</span><span class="token operator">/</span><span class="token keyword">INOUT</span> 参数名 参数类型 <span class="token punctuation">]</span><span class="token punctuation">)</span>
<span class="token keyword">BEGIN</span>
	<span class="token comment">-- SQL语句</span>
<span class="token keyword">END</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_5-case" tabindex="-1"><a class="header-anchor" href="#_5-case"><span>5)case</span></a></h4>
<p>case结构及作用，和我们在基础篇中所讲解的流程控制函数很类似。有两种语法格式：</p>
<p>语法一:</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 含义： 当case_value的值为 when_value1时，执行statement_list1，当值为 when_value2时，执行statement_list2， 否则就执行 statement_list</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>语法二:</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 含义： 当条件search_condition1成立时，执行statement_list1，当条件search_condition2成立时，执行statement_list2， 否则就执行 statement_list</span>
...
<span class="token punctuation">[</span><span class="token keyword">ELSE</span> statement_list<span class="token punctuation">]</span>
<span class="token keyword">END</span> <span class="token keyword">CASE</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>如果判定条件有多个，多个条件之间，可以使用 and 或 or 进行连接。</strong></p>
<h4 id="_6-while" tabindex="-1"><a class="header-anchor" href="#_6-while"><span>6)while</span></a></h4>
<p>while 循环是有条件的循环控制语句。满足条件后，再执行循环体中的SQL语句。具体语法为：</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 先判定条件，如果条件为true，则执行逻辑，否则，不执行逻辑</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_7-repeat" tabindex="-1"><a class="header-anchor" href="#_7-repeat"><span>7)repeat</span></a></h4>
<p>repeat是有条件的循环控制语句, 当满足until声明的条件的时候，则退出循环 。具体语法为：</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 先执行一次逻辑，然后判定UNTIL条件是否满足，如果满足，则退出。如果不满足，则继续下一次循环</span>
<span class="token keyword">REPEAT</span>
	<span class="token keyword">SQL</span>逻辑<span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span>
	UNTIL 条件
<span class="token keyword">END</span> <span class="token keyword">REPEAT</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_8-loop" tabindex="-1"><a class="header-anchor" href="#_8-loop"><span>8)loop</span></a></h4>
<p>LOOP 实现简单的循环，如果不在SQL逻辑中增加退出循环的条件，可以用其来实现简单的死循环。</p>
<p>LOOP可以配合一下两个语句使用：</p>
<ul>
<li>
...
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token punctuation">[</span>begin_label:<span class="token punctuation">]</span> <span class="token keyword">LOOP</span>
		<span class="token keyword">SQL</span>逻辑<span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_9-游标" tabindex="-1"><a class="header-anchor" href="#_9-游标"><span>9)游标</span></a></h4>
<p>游标（CURSOR）是用来存储查询结果集的数据类型 , 在存储过程和函数中可以使用游标对结果集进行循环的处理。游标的使用包括游标的声明、OPEN、FETCH 和 CLOSE，其语法分别如下。</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token comment">-- 游标声明</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="_10-条件处理程序" tabindex="-1"><a class="header-anchor" href="#_10-条件处理程序"><span>10)条件处理程序</span></a></h4>
<p>条件处理程序（Handler）可以用来定义在流程控制结构执行过程中遇到问题时相应的处理步骤。具体语法为：</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">DECLARE</span> handler_action <span class="token keyword">HANDLER</span> <span class="token keyword">FOR</span> condition_value <span class="token punctuation">[</span><span class="token punctuation">,</span> condition_value<span class="token punctuation">]</span>
...
<li>
<p><code v-pre>handler_action</code> 的取值：</p>
<ul>
<li>
<p><code v-pre>CONTINUE</code>: 继续执行当前程序</p>
</li>
<li>
<p><code v-pre>EXIT</code>: 终止执行当前程序</p>
</li>
</ul>
</li>
<li>
<p><code v-pre>condition_value</code> 的取值：</p>
<ul>
<li><code v-pre>SQLSTATE</code>: sqlstate_value: 状态码，如 02000</li>
<li><code v-pre>SQLWARNING</code>: 所有以01开头的SQLSTATE代码的简写</li>
<li><code v-pre>NOT FOUND</code>: 所有以02开头的SQLSTATE代码的简写</li>
<li><code v-pre>SQLEXCEPTION</code>: 所有没有被SQLWARNING 或 NOT FOUND捕获的SQLSTATE代码的简写</li>
</ul>
</li>
</ul>
<h3 id="_3、存储函数" tabindex="-1"><a class="header-anchor" href="#_3、存储函数"><span>3、存储函数</span></a></h3>
<p>存储函数是有返回值的存储过程，存储函数的参数只能是IN类型的。具体语法如下：</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">CREATE</span> <span class="token keyword">FUNCTION</span> 存储函数名称 <span class="token punctuation">(</span><span class="token punctuation">[</span> 参数列表 <span class="token punctuation">]</span><span class="token punctuation">)</span>
...
<h3 id="_4、触发器" tabindex="-1"><a class="header-anchor" href="#_4、触发器"><span>4、触发器</span></a></h3>
...
<ul>
<li>INSERT触发器:NEW 表示将要或者已经新增的数据</li>
<li>UPDATE触发器:OLD 表示修改之前的数据 , NEW 表示将要或已经修改后的数据</li>
<li>DELETE触发器:OLD 表示将要或者已经删除的数据</li>
</ul>
<p>创建:</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">CREATE</span> <span class="token keyword">TRIGGER</span> trigger_name
...
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">SHOW</span> TRIGGERS <span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>删除:</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">DROP</span> <span class="token keyword">TRIGGER</span> <span class="token punctuation">[</span>schema_name<span class="token punctuation">.</span><span class="token punctuation">]</span>trigger_name <span class="token punctuation">;</span> <span class="token comment">-- 如果没有指定 schema_name，默认为当前数据库</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>示例:</p>
<div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre v-pre class="language-sql"><code><span class="token keyword">create</span> <span class="token keyword">trigger</span> tb_user_insert_trigger
